...
> NOTE (implemented 2026-08-04): `VaultPermissions.pending_files()` returns
> `list[Path]`, not dicts. The panel maps each file via an `_ask_from_file`
> helper: `id` = file stem (request_id), `summary` = first fenced block in the
> ask md (missing/unreadable → `{"id": stem, "summary": ""}`). `render_permissions`
> stays pure per spec. The dict-shaped data exists only in the in-process
> `registry.pending()`, unusable across processes — the HUD reads the files.
