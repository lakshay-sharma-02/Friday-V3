...
```python
    ...
    def latest_notices(self, n: int = 5) -> list[dict]:
        ...
        out: list[dict] = []
        for p in self.notices()[:n]:
            try:
                text = p.read_text(encoding="utf-8").splitlines()
            except OSError:
                continue
            body = "\n".join(l for l in text if not l.startswith("- **"))
            out.append({
                "id": int(p.stem.split("-")[0]),
                "text": body.strip() or p.stem,
                "path": str(p),
                "at": p.stat().st_mtime,
            })
        return out
    ...
    def check(self) -> list[dict]:
        ...
        for p in sorted(self.notices_dir.glob("*.md")):
            ...
            if nid in self._seen:
                continue
            self._seen.add(nid)
            out.append({"id": nid, "text": p.stem, "path": str(p)})
        return out
