...
def evolve(conn, build_at: Optional[str] = None) -> int:
    ...
    def _record(entity_type, entity_id, event_type, previous_state, new_state, reason, metadata):
        nonlocal event_count
        insert_layer_history(
            conn, entity_type, entity_id, event_type,
    ...
    for kid, k in current_ids.items():
        ...
        meta = {
            "knowledge_type": k.type.value,
            "evidence_count": k.evidence_count,
            "evidence_ids": ",".join(sorted(ev_ids)),
            "confidence": k.confidence.value,
            "status": k.status.value,
        }
        ...
        if _conf_order(new_conf) > _conf_order(prev_conf):
            ...
            _record(
                "knowledge", kid, "Strengthened",
                previous_state=prev_h.status, new_state=k.status.value,
                reason=f"Confidence grew {prev_conf.value}->{new_conf.value} as evidence "
        ...
        if _status_rank(k.status) > _status_rank(KnowledgeStatus.from_str(prev_h.status)):
            et = "Verified" if k.status == KnowledgeStatus.VERIFIED else "Stabilized"
            _record(
                "knowledge", kid, et,
                previous_state=prev_h.status, new_state=k.status.value,
                reason=f"Lifecycle advanced {prev_h.status}->{k.status.value}.",
                metadata=meta,
            )
    ...
    # 3) Merge / Split detection (parents retained, children linked).
    _record_merge_split(_record, build_at, current_ids, prev, signals)

    # 4) Evidence-driven Dormant / Reactivated / Contradicted / Retired.
    _record_lifecycle(conn, _record, build_at, current_ids, prev, signals)
...
def _record_lifecycle(conn, record_fn, build_at, current_ids, prev, signals):
    """Dormant / Reactivated / Contradicted / Retired — ALL evidence-driven."""
    for kid, k in current_ids.items():
        sig = signals.get(k.subject)
        if not sig:
            continue
        # Latest evidence is a terminal removal -> Retired.
        if sig["retired"] and k.status != KnowledgeStatus.RETIRED:
            record_fn(
                "knowledge", kid, "Retired",
                previous_state=k.status.value, new_state=KnowledgeStatus.RETIRED.value,
                reason=f"'{k.subject}' shows a terminal removal/deprecation signal "
                f"(retired, not deleted); remains queryable for historical ask.",
                metadata={"evidence_ids": ",".join(sig["inactive_ids"])},
            )
            update_knowledge_status(conn, kid, KnowledgeStatus.RETIRED.value)
        # Latest evidence is quiet (usage stopped) -> Contradicted or Dormant.
            ...
            record_fn(
                "knowledge", kid, etype,
                previous_state=k.status.value, new_state=KnowledgeStatus.DORMANT.value,
                reason=f"Newer observation shows '{k.subject}' is no longer used "
                f"(removal/archived/inactive signal).",
                metadata={"evidence_ids": ",".join(sig["inactive_ids"])},
            )
    ...
    for kid, k in current_ids.items():
        if k.status in (KnowledgeStatus.DORMANT, KnowledgeStatus.RETIRED):
            ...
            if sig and sig["latest_active"]:
                record_fn(
                    "knowledge", kid, "Reactivated",
                    previous_state=k.status.value, new_state=KnowledgeStatus.OBSERVED.value,
                    reason=f"New usage observation for '{k.subject}'; reactivated.",
                    metadata={"evidence_ids": ",".join(sorted(set(k.evidence_ids)))},
                )
                update_knowledge_status(conn, kid, KnowledgeStatus.OBSERVED.value)


def _record_merge_split(
    record_fn, build_at: str,
    ...
    for kid, k in current_ids.items():
        ...
        if len(matched) >= 2:
            record_fn(
                "knowledge", kid, "Merged",
                previous_state=k.status.value, new_state=k.status.value,
                reason=f"Converges prior knowledge: {', '.join(matched)}.",
                metadata={"related": ",".join(sorted(matched))},
            )

    # Split: a prior belief went quiet or vanished while >=2 NEW subjects appeared.
    new_subjects = {
    ...
    for pid, ph in prev.items():
        ...
        if went_quiet and len(children) >= 2:
            record_fn(
                "knowledge", pid, "Split",
                previous_state=ph.status, new_state=KnowledgeStatus.RETIRED.value,
                reason=f"Prior knowledge split into: {', '.join(children)}.",
                metadata={"related": ",".join(sorted(children))},
            )
...
def evolution_timeline(conn) -> list[dict]:
    """All knowledge evolution events, newest first."""
    rows = conn.execute(
