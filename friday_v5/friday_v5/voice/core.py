...
def config_from_env() -> PipelineConfig:
    """Build a PipelineConfig from env overrides (V5 — no JSON config
    file; defaults are the built-ins, never raises)."""
    import os
    try:
        return PipelineConfig(
            hotword=os.environ.get("FRIDAY_V5_HOTWORD", "hey friday"),
            hotword_sensitivity=float(os.environ.get(
                "FRIDAY_V5_HOTWORD_SENSITIVITY", "0.7")),
            vad_mode=int(os.environ.get("FRIDAY_V5_VAD_MODE", "1")),
            stt_model=os.environ.get("FRIDAY_V5_STT_MODEL", "base.en"),
            tts_provider=os.environ.get("FRIDAY_V5_TTS_PROVIDER", "piper"),
            tts_voice=os.environ.get("FRIDAY_V5_TTS_VOICE", ""),
            tts_speed=float(os.environ.get("FRIDAY_V5_TTS_SPEED", "1.15")),
        )
    except Exception:
        return PipelineConfig()

logger = logging.getLogger("friday_v5.voice.core")
