...
from friday_v4.agent import (ClaudeBridge, CLAUDE_END, CLAUDE_PREFIX,
                             get_bridge, is_claude_end, is_claude_message)
from friday_v4.agent import permissions as P
...
class _FakeClient:
    ...
    async def receive_messages(self):
        for m in self._messages:
            yield m
        while True:
            await asyncio.sleep(3600)  # stay open like the real stream
...
def _fresh_state(monkeypatch):
    ...
    import friday_v4.agent.bridge as B
...
class TestTalkRouting:
    ...
    def test_agent_status_endpoint(self, monkeypatch, tmp_path):
        ...
        api = self._api(tmp_path)
        st = api.agent_status()
        assert set(st) == {"available", "active", "busy"}
...
class TestPermissionWeave:
    ...
    def test_double_resolve_is_noop(self, db_path):
        ...
        from friday_v4.agent.permissions import BridgePermission
