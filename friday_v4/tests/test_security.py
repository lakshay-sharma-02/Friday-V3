...
from friday_v4.security import (
    DependencyAuditor,
    QualityGate,
    SecretDetector,
    SecurityReport,
...
from friday_v4.security.reporter import SEVERITY_ORDER, Finding, severity_color
from friday_v4.security.tooling import find_tool, tool_available
...
def _hermetic_builtin_only(monkeypatch):
    ...
    """
    import friday_v4.security.deps as deps
    import friday_v4.security.quality as quality
    import friday_v4.security.secrets as secrets
...
class TestDepsParsing:
    ...
    def test_version_tuple_ordering(self):
        from friday_v4.security.deps import version_lt, version_tuple
    ...
    def test_parse_requirements_comments_and_flags(self):
        from friday_v4.security.deps import parse_requirements
...
class TestOptionalTools:
    ...
    def test_scan_with_ruff_parses_json(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(
            "friday_v4.security.quality.find_tool", lambda name: "/usr/bin/ruff")
    ...
    def test_scan_with_mypy_parses_json(self, monkeypatch, tmp_path):
        ...
        self._fake_run(monkeypatch, payload)
        monkeypatch.setattr(
            "friday_v4.security.quality.find_tool", lambda name: "/usr/bin/mypy")
    ...
    def test_scan_with_trufflehog_parses_verified(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(
            "friday_v4.security.secrets.find_tool", lambda name: "/usr/bin/trufflehog")
    ...
    def test_scan_with_pip_audit_parses_json(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(
            "friday_v4.security.deps.tool_available", lambda name: True)
        req = tmp_path / "requirements.txt"
        req.write_text("requests==2.30.0\n")
    ...
    def test_scan_with_pip_audit_absent_returns_empty(self, monkeypatch):
        monkeypatch.setattr(
            "friday_v4.security.deps.tool_available", lambda name: False)
...
class TestSecurityCLI:
    def test_parser_registers_subcommands(self):
        ...
        from friday_v4.cli_security import build_security_parser
    ...
    def test_integrated_friday4_parser_includes_security(self):
        """The `friday4` entry point exposes `friday4 security`."""
        import argparse

        from friday_v4.cli_daemon import build_daemon_parser
