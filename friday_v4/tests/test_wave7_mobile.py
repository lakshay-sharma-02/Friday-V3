...
class TestPairing:
    ...
    def service(self, tmp_path, monkeypatch):
        from friday_v4.mobile.pairing import PairingService
...
class TestDeviceAPI:
    ...
    def test_register_device_endpoint(self, server, tmp_path, monkeypatch):
        from friday_v4.mobile.pairing import PairingService
        monkeypatch.setattr("friday_v4.mobile.pairing._DEFAULT_STATE",
                            tmp_path / "pair.json")
        service = PairingService(db_path=tmp_path / "v4.db",
                                 state_file=tmp_path / "pair.json")
        code = service.generate()
...
class TestExpoTransporter:
    def test_posts_to_expo(self, monkeypatch):
        from friday_v4.mobile import Notification, expo_transporter
        from friday_v4.mobile.push import EXPO_PUSH_URL
        sent = {}
    ...
    def test_network_failure_never_raises(self, monkeypatch):
        from friday_v4.mobile import Notification, expo_transporter
        monkeypatch.setattr("friday_v4.mobile.push._post",
                            lambda *a, **k: None)
...
class TestFanout:
    def test_pushes_to_every_paired_device(self, tmp_path, monkeypatch):
        from friday_v4.mobile import Notification, fanout_transporter
        dbp = tmp_path / "v4.db"
        conn = db.connect(dbp)
        ...
        monkeypatch.setattr("friday_v4.mobile.push._post", _fake_post)
...
class TestPhoneDesktopControl:
    ...
    CLI (``friday4 talk``) and the web dashboard use — Friday focuses /
    ...
    def test_talk_routes_open_brave_to_desktop(self, tmp_path, monkeypatch):
        ...
        from friday_v4.mobile.api import MobileAPI
...
class TestBindNormalization:
    ...
    (exactly what `friday4 mobile remote` prints) and got
    ``Name or service not known`` — the CLI must not die on that."""

    def _norm(self, host, port=8900):
        from friday_v4.cli_mobile import _normalize_bind
...
class TestAutostart:
    """`friday4 mobile autostart|no-autostart` — the 9router-style XDG
    ...
    def test_autostart_writes_entry(self, monkeypatch, tmp_path, capsys):
        ...
        path = cfg / "autostart" / "friday4-mobile.desktop"
        assert path.exists()
    ...
    def test_autostart_bakes_token_and_tunnel(self, monkeypatch, tmp_path,
        ...
        content = (cfg / "autostart" /
                   "friday4-mobile.desktop").read_text()
...
class TestCloudflareTunnel:
    ...
    def test_spawn_returns_none_without_cloudflared(self, monkeypatch):
        import friday_v4.cli_mobile as cli
...
class TestMobileTray:
    ...
    def test_tray_unavailable_without_pystray(self, monkeypatch):
        ...
        from friday_v4.mobile.tray import MobileTray
...
class TestMobileRemoteCLI:
    """`friday4 mobile remote` — the anywhere-access answer (hermetic:
...
class TestCLI:
    def test_pair_devices_unpair_roundtrip(self, tmp_path, capsys,
        ...
        from friday_v4.mobile import PairingService
        service = PairingService(db_path=tmp_path / "v4.db",
                                 state_file=tmp_path / "pair.json")
        code = service.generate()
    ...
    def test_mobile_exports(self):
        from friday_v4.mobile import (PairingService, expo_transporter,
                                      fanout_transporter)
