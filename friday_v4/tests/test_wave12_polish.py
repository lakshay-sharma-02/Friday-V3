...
class TestSSHExecutor:
    def test_ssh_registered(self):
        from friday_v4.execution.executors import _EXECUTORS, SSHExecutor
        assert _EXECUTORS["ssh"] is SSHExecutor
        from friday_v4.network import ssh_available
        assert ssh_available() is True

    def test_ssh_gate_confirm_by_default(self):
        from friday_v4.execution.gate import PermissionLevel, PermissionGate
        gate = PermissionGate()
    ...
    def test_ssh_undo_none(self):
        from friday_v4.execution.executors import SSHExecutor
        ex = SSHExecutor()
    ...
    def test_ssh_parses_host_and_remote(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "friday_v4.execution.executors.find_tool", lambda name: "/usr/bin/ssh")
