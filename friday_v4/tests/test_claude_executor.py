...
- the CLI surface (`friday4 execute claude "<task>"`) accepts the type
...
class TestRegistration:
    def test_claude_registered(self):
        from friday_v4.execution.executors import _EXECUTORS, ClaudeCodeExecutor
        assert _EXECUTORS["claude"] is ClaudeCodeExecutor

    def test_gate_confirm_by_default(self):
        from friday_v4.execution.gate import PermissionGate, PermissionLevel
        gate = PermissionGate()
        assert gate.level_for("claude", "figure out why the build fails") \
            == PermissionLevel.CONFIRM

    def test_gate_destructive_task_escalates(self):
        from friday_v4.execution.executors import _CLAUDE_DANGEROUS_PHRASES
        from friday_v4.execution.gate import PermissionGate, PermissionLevel
...
def test_hang_times_out(tmp_path, fake_claude):
    from friday_v4.execution.executors import ClaudeCodeExecutor
    from friday_v4.execution.sandbox import Sandbox
...
# ─────────────────────────────────────────────────────────────────────
# CLI surface — friday4 execute claude "<task>"
# ─────────────────────────────────────────────────────────────────────


class TestCliExecute:
    def test_claude_choice_parses(self, tmp_path, fake_claude):
        from friday_v4.cli_execute import main as execute_main
        code = execute_main([
