"""Tests for the `friday4 collab` CLI surface."""
...
def fake_coordinator(monkeypatch):
    monkeypatch.setattr("friday_v4.cli_collab.Coordinator",
                        _FakeCoordinator)
...
class TestCollabCLIParser:
    def test_parser_registers_subcommands(self):
        from friday_v4.cli_collab import build_collab_parser
...
class TestCollabCLICommands:
    def test_status_command(self, fake_coordinator):
        from friday_v4.cli_collab import main
    ...
    def test_wired_into_friday4_main(self, fake_coordinator):
        """`friday4 collab status` routes through the main CLI dispatch."""
        from friday_v4 import cli_talk
