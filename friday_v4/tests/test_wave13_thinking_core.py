...
- **Conversation-capable:** history is threaded into the synthesis
  prompt; ``friday4 ask`` logs the Q&A exchange.
- **Wiring:** the engine applies the post-pass; ``friday4 ask``,
  ``friday4 talk``/voice, and web all inherit it via ``answer()``.
...
from friday_v4.reasoning import answer, llm_provider, parse
from friday_v4.reasoning.evidence import Answer, Evidence
from friday_v4.reasoning.question import QuestionType
...
class TestEngineEnhancement:
    ...
    def test_env_opt_in_uses_client(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("FRIDAY_V4_LLM", "1")
        from friday_v4 import nlu as nlu_mod
        conn = _seed(tmp_path)
        try:
...
class TestCLIAsk:
    def test_ask_logs_exchange(self, tmp_path, monkeypatch):
        """friday4 ask records the Q&A in the conversation log (Wave 13)."""
        monkeypatch.delenv("FRIDAY_V4_LLM", raising=False)
        from friday_v4.cli_ask import cmd_ask
        conn = _seed(tmp_path)
        conn.close()
