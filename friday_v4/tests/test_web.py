"""Tests for the `friday4 web` dashboard — data payloads + HTTP server.
...
class TestDashboardData:
    ...
    def test_intelligence_state_graceful_without_stores(self, tmp_path,
        ...
        monkeypatch.setattr(
            "friday_v4.intelligence.drift._DRIFT_FILE",
            tmp_path / "none" / "drift.json")
        monkeypatch.setattr(
            "friday_v4.intelligence.anomaly._ANOMALY_FILE",
    ...
    def test_autonomy_state_reads_pending_read_only(self, tmp_path,
        ...
        monkeypatch.setattr("friday_v4.db.connect",
    ...
    def test_autonomy_approve_deny_route_through_agent(self, tmp_path,
        ...
        monkeypatch.setattr("friday_v4.autonomy.AutonomyAgent", _FakeAgent)
    ...
    def test_memory_state_reads_facts_read_only(self, tmp_path,
        ...
        from friday_v4 import db
        from friday_v4.memory import FactMemory, WorkingMemory
        dbp = tmp_path / "v4.db"
        conn = db.connect(dbp)
    ...
    def test_talk_returns_talk_result_payload(self, monkeypatch):
        ...
        # so friday_v4.execution.execute is the true resolution point.
        monkeypatch.setattr("friday_v4.db.connect",
                            lambda **kw: _FakeConn())
        monkeypatch.setattr("friday_v4.execution.execute",
...
class TestWebServer:
    ...
    def test_api_projects_returns_directory_list(self, server):
        ...
        assert any(p.endswith("friday_v4") for p in projects)
    ...
    def test_scan_endpoint_starts_background_scan(self, server, monkeypatch):
        ...
        from friday_v4 import daemon as daemon_mod
    ...
    def test_scan_endpoint_rejects_concurrent_scan(self, server, monkeypatch):
        ...
        from friday_v4 import daemon as daemon_mod
        from friday_v4.web import server as server_mod
    ...
    def test_talk_endpoint_returns_response(self, server, monkeypatch):
        ...
        import friday_v4.web.dashboard as dash
