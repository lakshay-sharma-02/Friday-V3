"""CLI commands for `friday4 execute` / `friday4 actions` — Wave 9 execution.
...
Usage:
    friday4 execute <action_type> <command...> [--cwd DIR] [--db PATH]
                    [--force] [--yes] [--json] [--goal TEXT]
    friday4 actions [--limit N] [--type T] [--db PATH] [--json]

Action types: shell | git | file | python | testing | ssh (Wave 12) |
claude (Wave 18 — delegate a complex task to the Claude Code CLI, e.g.
`friday4 execute claude "figure out why the build fails and fix it"`)
...
including denied attempts. `friday4 actions` reads it back.
"""
...
logger = logging.getLogger("friday_v4.cli_execute")
...
def cmd_actions(args: argparse.Namespace) -> int:
    ...
    if not rows:
        _print_dim("no actions recorded yet — run `friday4 execute` first")
        return EXIT_OK
...
def build_execute_parser(subparsers) -> None:
    """Register `friday4 execute` and `friday4 actions`."""
...
def main(argv: list[str] | None = None) -> int:
    """Standalone entry point for `python -m friday_v4.cli_execute`."""
    logging.basicConfig(level=logging.WARNING)

    parser = argparse.ArgumentParser(prog="friday4 execute")
