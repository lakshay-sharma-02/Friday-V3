"""CLI commands for `friday4 relationship` — how close Friday and you are (Wave 10 §3.3).
...
Usage:
    friday4 relationship status              # depth, level, tone, signals
    friday4 relationship status --json       # machine-readable
    friday4 relationship refresh             # recompute + persist now
"""
...
logger = logging.getLogger("friday_v4.cli_relationship")
...
def cmd_relationship_status(args: argparse.Namespace) -> int:
    """`friday4 relationship status` — the relationship view."""
...
def cmd_relationship_tone(args: argparse.Namespace) -> int:
    """`friday4 relationship tone [tone] [--verbosity N] [--reset]`.

    Wave 17 adaptive identity: set/clear the explicit tone-direction
    (the CLI is a debug hatch — "be more casual" through `friday4 talk`
    ...
    try:
        ...
        if getattr(args, "reset", False):
            ...
            status = engine.set_direction(tone=tone, verbosity=verbosity,
                                          request="friday4 relationship tone")
            changed = "set"
    except Exception as exc:
...
def main(argv: list[str] | None = None) -> int:
    """Standalone entry point for `python -m friday_v4.cli_relationship`."""
    logging.basicConfig(level=logging.WARNING)
    parser = argparse.ArgumentParser(prog="friday4 relationship")
