...
logger = logging.getLogger("friday_v4.collab.coordinator")
...
class Coordinator:
    ...
    def peers(self) -> list[PeerInfo]:
        ...
        ``friday4 collab peers``/``status`` run without starting discovery,
    ...
    def status(self) -> dict:
        """Snapshot for ``friday4 collab status`` (never raises)."""
        return {
            "peer_id": self.peer_id,
