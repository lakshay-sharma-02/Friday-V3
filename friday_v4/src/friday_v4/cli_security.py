"""CLI commands for `friday4 security` — Wave 3 security & quality.

Usage:
    friday4 security scan [path]               # scan a project (default: cwd)
    friday4 security scan --json               # machine-readable output
    friday4 security scan --threshold high     # only critical/high findings
    friday4 security scan --no-quality         # skip the quality gate
...
logger = logging.getLogger("friday_v4.cli_security")
...
def build_security_parser(subparsers) -> None:
    """Build the `friday4 security` subparser (used by the integrated CLI)."""
...
def main(argv: list[str] | None = None) -> int:
    """Standalone entry point for `python -m friday_v4.cli_security`."""
    logging.basicConfig(level=logging.WARNING)

    parser = argparse.ArgumentParser(prog="friday4 security")
