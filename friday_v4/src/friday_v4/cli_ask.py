"""CLI command for `friday4 ask` — evidence-cited answers (Wave 9).
...
Same conventions as `friday4 execute` / `friday4 talk` (colors, exit
codes, JSON purity).

Usage:
    friday4 ask "what's the status of my projects?"
    friday4 ask "what did I do recently?" --json
"""
...
logger = logging.getLogger("friday_v4.cli_ask")
...
def _recent_history(conn) -> list[dict]:
    ...
    ``friday4 ask`` is conversation-capable (Wave 13): a follow-up like
...
def cmd_ask(args: argparse.Namespace) -> int:
    """`friday4 ask <question...>` — answer with cited evidence."""
    if not args.question:
        _print_error("ask needs a question, e.g. friday4 ask \"what's the "
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(prog="friday4 ask")
