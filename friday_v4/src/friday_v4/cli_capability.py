"""CLI commands for `friday4 capability` — what Friday can do (Wave 16, Law 7).

The debug-hatch surface for the capability registry (the product path
is `friday4 talk "what can you do"`). Lists, describes, and counts the
registered capabilities — builtins (executors, providers, intents,
surfaces) plus learned skills (self-extension).

Usage:
    friday4 capability list                 # all registered capabilities
    friday4 capability list --json
    friday4 capability describe executor:shell
    friday4 capability count
"""
...
logger = logging.getLogger("friday_v4.cli_capability")
...
def cmd_capability_list(args: argparse.Namespace) -> int:
    """`friday4 capability list` — everything Friday can do."""
...
def cmd_capability_describe(args: argparse.Namespace) -> int:
    """`friday4 capability describe <id>` — one capability in detail."""
    ...
    if not description:
        print(f"  {_RED}✗ no capability with id {cap_id!r}.{_RESET}\n"
              f"  {_DIM}  Run 'friday4 capability list' to see them.{_RESET}")
        return EXIT_USAGE
...
def cmd_capability_count(args: argparse.Namespace) -> int:
    """`friday4 capability count` — how many things Friday can do."""
    conn = _resolve_db(args)
    try:
        from .capability import CapabilityRegistry
...
def main(argv: list[str] | None = None) -> int:
    """Standalone entry point for `python -m friday_v4.cli_capability`."""
    logging.basicConfig(level=logging.WARNING)
    parser = argparse.ArgumentParser(prog="friday4 capability")
