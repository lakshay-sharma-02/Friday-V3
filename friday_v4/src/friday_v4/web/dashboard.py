"""Dashboard data — JSON payloads for the `friday4 web` UI.
...
logger = logging.getLogger("friday_v4.web.dashboard")
...
def intelligence_state() -> dict:
    ...
    try:
        from friday_v4.intelligence import AnomalyDetector, DriftPredictor
        out.update({
            "available": True,
...
def talk(text: str) -> dict:
    ...
    same command language as voice and ``friday4 talk``: ``resolve`` →
    ...
    try:
        ...
        try:
            llm = None
            try:
                from friday_v4.nlu import LLMClient
                llm = LLMClient()
            except Exception:
                llm = None
