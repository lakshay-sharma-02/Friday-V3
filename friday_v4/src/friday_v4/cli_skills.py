"""CLI commands for `friday4 skills` — learned workflows, shadow-first (Wave 10 §3.4).
...
Usage:
    friday4 skills list                    # all skills + state
    friday4 skills learn                   # form shadow skills from patterns
    friday4 skills shadow                  # record shadow matches now
    friday4 skills promote <name>          # operator approval → promoted
    friday4 skills status                  # summary counts by state
    friday4 skills watch [name]            # Wave 14: watch me do this
    friday4 skills watch-stop [name]       # Wave 14: learn this → skill
    friday4 skills noticed                 # Wave 14: 'I noticed you do this every time'
    friday4 skills dispatch                # next-step suggestions on context match
"""
...
logger = logging.getLogger("friday_v4.cli_skills")
...
def cmd_skills_list(args: argparse.Namespace) -> int:
    """`friday4 skills list` — all skills with verification state."""
    ...
    _print_logo()
    if not skills:
        print(f"  {_DIM}No skills yet — run 'friday4 skills learn' to form "
...
def cmd_skills_learn(args: argparse.Namespace) -> int:
    """`friday4 skills learn` — form shadow skills from the audit log."""
    conn = _resolve_db(args)
    try:
...
def cmd_skills_shadow(args: argparse.Namespace) -> int:
    """`friday4 skills shadow` — record shadow matches now (no execution)."""
    conn = _resolve_db(args)
    try:
...
def cmd_skills_promote(args: argparse.Namespace) -> int:
    """`friday4 skills promote <name>` — the operator-approval step."""
    name = (args.name or "").strip()
    if not name:
        print(f"  {_RED}✗ give me a skill name, e.g. "
              f"'friday4 skills promote run-tests'.{_RESET}")
        return EXIT_USAGE
    conn = _resolve_db(args)
    try:
        from .skills import SkillRegistry
        ...
        if skill.verification_state != "verified":
            print(f"  {_YELLOW}⚠ '{name}' is {skill.verification_state} — only "
                  f"verified skills can be promoted. Run 'friday4 skills "
...
def cmd_skills_watch(args: argparse.Namespace) -> int:
    """`friday4 skills watch` — start an explicit demonstration capture."""
    conn = _resolve_db(args)
    try:
    ...
    print(f"  {_GREEN}● Watching{name} — go ahead. When you're done, run "
          f"'friday4 skills watch-stop'.{_RESET}")
    print()
    return EXIT_OK


def cmd_skills_watch_stop(args: argparse.Namespace) -> int:
    """`friday4 skills watch-stop` — form a skill from what was watched."""
    conn = _resolve_db(args)
    try:
        from .skills import WatchRecorder
        watcher = WatchRecorder(conn)
    ...
    if formed is None:
        ...
        if not active:
            print(f"  {_YELLOW}⚠ I wasn't watching anything — run "
                  f"'friday4 skills watch' first.{_RESET}")
        else:
...
def cmd_skills_noticed(args: argparse.Namespace) -> int:
    """`friday4 skills noticed` — 'I noticed you do this every time'."""
    conn = _resolve_db(args)
    try:
    ...
    for offer in offers:
        ...
        print(f"    {_DIM}→ 'friday4 skills watch' then do it, or say "
              f"'learn this'.{_RESET}")
    print()
    return EXIT_OK
...
def main(argv: list[str] | None = None) -> int:
    """Standalone entry point for `python -m friday_v4.cli_skills`."""
    logging.basicConfig(level=logging.WARNING)
    parser = argparse.ArgumentParser(prog="friday4 skills")
