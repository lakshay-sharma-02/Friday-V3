"""CLI commands for `friday4 memory` — long-term facts + working context (Wave 10).
...
memory context. Same conventions as `friday4 ask` / `friday4 talk`
(colors, exit codes, JSON purity).

Usage:
    friday4 memory store operator.prefers_rust True --source voice:2026-08-01
    friday4 memory recall operator.prefers_rust
    friday4 memory forget operator.name
    friday4 memory list
    friday4 memory status          # counts + working memory context
"""
...
logger = logging.getLogger("friday_v4.cli_memory")
...
def cmd_memory_store(args: argparse.Namespace) -> int:
    """`friday4 memory store <key> <value>` — remember a fact."""
...
def cmd_memory_recall(args: argparse.Namespace) -> int:
    """`friday4 memory recall <key>` — recall one fact with provenance."""
    subject, predicate = _split_key(args.key)
    conn = _resolve_db(args)
    try:
        from .memory import FactMemory
...
def cmd_memory_forget(args: argparse.Namespace) -> int:
    """`friday4 memory forget <key>` — delete a fact."""
    subject, predicate = _split_key(args.key)
    conn = _resolve_db(args)
    try:
        from .memory import FactMemory
...
def cmd_memory_list(args: argparse.Namespace) -> int:
    """`friday4 memory list` — all facts, optionally by subject."""
    conn = _resolve_db(args)
    try:
        from .memory import FactMemory
    ...
    if not facts:
        _print_friday("I don't remember anything yet. "
                      "Try 'friday4 memory store operator.name Lakshay'.")
        print()
        return EXIT_OK
...
def cmd_memory_status(args: argparse.Namespace) -> int:
    """`friday4 memory status` — fact count + working memory context."""
    conn = _resolve_db(args)
    try:
...
def main(argv: list[str] | None = None) -> int:
    """Standalone entry point for `python -m friday_v4.cli_memory`."""
    logging.basicConfig(level=logging.WARNING)
    parser = argparse.ArgumentParser(prog="friday4 memory")
