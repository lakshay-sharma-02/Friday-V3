...
operators direct access):
    ...
    friday4 analyze <repo>                 # repo architecture analysis
    friday4 correlate <a> <b>              # cross-project integration cost
    friday4 briefing [morning|evening]     # briefing from real V4 state
    friday4 narrative [date]               # the day's story from the audit log
    friday4 report <title> <key=value...>  # deterministic cited report

Same conventions as every other `friday4` command: colors, exit codes,
JSON purity.
"""
...
logger = logging.getLogger("friday_v4.cli_research")
...
def cmd_analyze(args: argparse.Namespace) -> int:
    """`friday4 analyze <repo>` — evidence-cited repo analysis."""
...
def cmd_briefing(args: argparse.Namespace) -> int:
    """`friday4 briefing [morning|evening]` — briefing from real state."""
...
def cmd_narrative(args: argparse.Namespace) -> int:
    """`friday4 narrative [date]` — the day's story from real state."""
    conn = _resolve_db(args)
    try:
...
def cmd_report(args: argparse.Namespace) -> int:
    ...
    if not sections:
        _print_error("report needs sections: friday4 report 'Security' "
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(prog="friday4 research")
    # Standalone entry (used by ``friday4 research`` in cli_nl): the
