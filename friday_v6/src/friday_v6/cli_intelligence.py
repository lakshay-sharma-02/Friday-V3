"""CLI commands for `friday6 intelligence` — advanced intelligence layer.

Usage:
    friday6 intelligence status         # Overview of drift/anomaly/health/predictions
    friday6 intelligence health [path]  # Code health diagnostics for a project
    friday6 intelligence predict        # Predictive insights on tracked metrics
    friday6 intelligence anticipate     # What's likely to break / drift next
    friday6 intelligence drift --metric NAME --value N
    friday6 intelligence anomaly --category NAME --value N
    friday6 intelligence learn --category NAME --feedback positive|negative
"""
...
logger = logging.getLogger("friday_v6.cli_intelligence")
...
def cmd_intelligence_status(args: argparse.Namespace) -> int:
    ...
    if drift_stats["metrics_tracked"]:
        ...
        _print_dim("No metrics tracked yet — record samples via `friday6 intelligence drift`.")
    ...
    if learner_stats["categories_learned"]:
        ...
        _print_dim("No correction feedback yet — record via `friday6 intelligence learn`.")
...
def cmd_intelligence_predict(args: argparse.Namespace) -> int:
    ...
    if not metrics:
        ...
        _print_dim("  friday6 intelligence drift --metric NAME --value N")
        print()
        return 0
...
def build_intelligence_parser(subparsers) -> None:
    """Build the `friday6 intelligence` subparser (used by the integrated CLI)."""
...
def main(argv: list[str] | None = None) -> int:
    """Standalone entry point for `python -m friday_v6.cli_intelligence`."""
    logging.basicConfig(level=logging.WARNING)

    parser = argparse.ArgumentParser(prog="friday6 intelligence")
