"""CLI commands for `friday6 collab` — Wave 5 collaboration.

Usage:
    friday6 collab start [--workspace NAME] [--port N] [--beacon-port N]
                         # run a coordinator (sync + discovery)
    friday6 collab status                                # local snapshot
    friday6 collab peers                                 # discovered peers
    friday6 collab observations [--limit N]              # merged observations
    friday6 collab add <json-or-text>                    # record an observation
    friday6 collab share [--host H --port P]             # one-shot sync
    friday6 collab perms <list|add|remove> ...
...
logger = logging.getLogger("friday_v6.cli_collab")
...
def cmd_collab_perms(args: argparse.Namespace) -> int:
    ...
    if action == "list":
        ...
        if not coordinator.permissions.members():
            _print_dim("no members — add one with "
                       "`friday6 collab perms add <peer> <role>`")
        return 0
    if action == "add":
        if not args.peer_id:
            _print_error("usage: friday6 collab perms add <peer> <role>")
            return 1
    ...
    if action == "remove":
        if not args.peer_id:
            _print_error("usage: friday6 collab perms remove <peer>")
            return 1
...
def _add_collab_commands(collab_parser) -> None:
    ...
    Shared by the ``friday6 collab`` dispatch (via build_collab_parser)
    and the standalone ``python -m friday_v6.cli_collab`` entry, which
    already *is* ``friday6 collab`` — so it must not nest one more
    ``collab`` level.
    """
...
def build_collab_parser(subparsers) -> None:
    """Attach the ``collab`` subcommand to the ``friday6`` parser."""
...
def main(argv: list[str] | None = None) -> int:
    """Standalone entry point for `python -m friday_v6.cli_collab`."""
    logging.basicConfig(level=logging.WARNING)

    parser = argparse.ArgumentParser(prog="friday6 collab")
