...
the tests"* is resolved by the ONE NLU point (:mod:`friday_v6.nlu`
...
- ``friday6 talk "run the tests"`` (see ``cli_nl.py``)
...
logger = logging.getLogger("friday_v6.nl_router")
...
#: "what do you know about X" — vault/memory questions answered from
#: evidence (Wave 1). Conservative: only these explicit phrases.
_VAULT_ASK_PHRASES = (
    "what do you know about ",
    "what do you remember about ",
    "what have you got on ",
    "what's in your vault about ",
    "what is in your vault about ",
    "what do you have about ",
)


#: "what skills do you have" family — Wave 2 discoverability phrases
#: routed pre-dispatch (they can classify ASK *or* unknown depending on
#: phrasing, so the overview must win before intent dispatch).
_SKILL_OVERVIEW_PHRASES = (
    "what skills do you have",
    "what skills do you got",
    "what skills can you use",
    "what are your skills",
    "list your skills",
    "show me your skills",
    "what can you do with skills",
    "what markdown skills",
)


def _is_skill_overview_phrase(text: str) -> bool:
    """Whether an utterance asks to list Friday's skills (Wave 2)."""
    lower = (text or "").lower()
    return any(p in lower for p in _SKILL_OVERVIEW_PHRASES)


#: Vault phrases (Wave 0 → NL, follow-up): the vault is operator memory
#: and must answer from *language*, not just ``friday6 vault find``.
#: ``_vault_nl_response`` runs pre-dispatch because "find X in my vault"
#: reads as DESKTOP to the resolver (the "find" keyword) and "search the
#: vault for X" as PLAN — both wrong. "what do you know about X" stays
#: the ASK path (evidence-cited, Wave 1).
_VAULT_SEARCH_RE = re.compile(
    r"(?:find|look up|search for|grep)\s+(.+?)\s+in\s+(?:my|the)\s+vault"
    r"|(?:search|look in|grep)\s+(?:my|the)\s+vault\s+for\s+(.+)",
    re.IGNORECASE)
_VAULT_LIST_PHRASES = (
    # Anchored with an action verb — bare substrings like "my vault
    # notes" would swallow unrelated utterances ("remember that my
    # vault notes are important" must stay MEMORY).
    "list my vault notes", "list the vault", "list my vault",
    "show me the vault", "show my vault notes", "what's in my vault",
    "what is in my vault", "what have i stored",
    "what's in the vault", "what is in the vault", "what's stored",
)
_INDEX_REBUILD_PHRASES = (
    "rebuild the index", "rebuild the vault index", "reindex the vault",
    "rebuild index", "refresh the index",
)


def _vault_search_terms(text: str) -> str:
    """The search terms from a vault-search utterance, or ""."""
    m = _VAULT_SEARCH_RE.search(text or "")
    if m:
        return (m.group(1) or m.group(2) or "").strip()
    return ""


#: Skill self-creation (Wave 2 follow-up): "create a skill called X that
#: does Y" authors a SKILL.md into the operator skills dir — Friday
#: extending itself in language. Conservative: must name the skill.
_SKILL_CREATE_RE = re.compile(
    r"(?:create|make|add|write)\s+(?:a\s+|an\s+|another\s+|new\s+)*"
    r"skill\s+(?:called|named|for)\s+([\w-]+)",
    re.IGNORECASE)


def _skill_create_name(text: str) -> str:
    """The skill name from a create-skill utterance, or ""."""
    m = _SKILL_CREATE_RE.search(text or "")
    return m.group(1).strip() if m else ""


def _skill_body_from_description(name: str, description: str) -> str:
    """Build the SKILL.md body for a self-created skill (Wave 2).

    The old stub wrote only description + generic rules — a shell that
    gave the Claude bridge nothing to follow ("use the xyz skill"
    routed but nothing ran). Now the description is split into an
    explicit ``## Steps`` checklist so both the Claude path and the
    deterministic floor have an actionable how-to. The description
    phrase is preserved verbatim as the goal; the steps are a best-
    effort decomposition of its clauses ("open X and do Y" → two
    steps). No fabrication: when nothing decomposes, the steps section
    says the description *is* the goal and the operator should add
    detail.
    """
    # Split on coordinating conjunctions that usually separate actions
    # ("opens workspace 5 and opens terminal in it" → two steps).
    clauses = [c.strip(" .,;:") for c in re.split(
        r"\s+(?:and|then|afterwards|next)\s+", description)
        if c.strip(" .,;:")]
    clauses = [c for c in clauses if c]
    if not clauses:
        clauses = [description]
    steps = "\n".join(f"{i}. {c.capitalize()}" if not c[0].isupper()
                      else f"{i}. {c}"
                      for i, c in enumerate(clauses, 1))
    note = ("The description is the goal; if these steps are not exact, "
            "edit this file to make them precise.") if len(clauses) > 1 \
        else ("This is a single-step description; add more steps by "
              "editing this file if the task needs them.")
    return (
        ...
        f"- **Report honestly.** Say what ran and the outcome.\n"
        f"- Save useful artifacts to `~/.friday/v6_vault/outputs/`.\n")


#: "teach me to do X" (Wave 3 follow-up) — the watch-me loop closes in
#: natural language. When Friday doesn't know something, the operator
#: *teaches* by doing: "teach me to do deploy" opens a demonstration
#: capture; "that's it" / "done teaching" closes it and auto-forms a
#: shadow skill from the audited actions (same WatchRecorder as "watch
#: me do this" — shadow-first, never executes until verified+promoted).
#: Start phrases must be conservative: "teach me to remember that X" is
#: MEMORY fact storage (not a demo) and "show me X" is ASK — neither
#: may open a capture.
_TEACH_START_RE = re.compile(
    r"(?:teach me (?:how to |to do |to )?|"
    r"(?:i'?ll|i will|let me|i'?m going to|im going to)\s+show you "
    r"(?:how to (?:do )?|how )?|show you how to (?:do )?)(.+)$",
    re.IGNORECASE)

#: Completion phrases for a teaching session. ``strong`` phrases answer
#: honestly even when idle ("stop teaching" with no capture is a
#: meaningful request); ``generic`` phrases ("that's it") only fire when
#: a capture is actually open — a bare "that's it" in ordinary chat is
#: never hijacked. Both sets match the WHOLE utterance ("i'm done with
#: the vault" stays MEMORY, not a lesson end), EXCEPT the strong verbs
#: which may carry a trailing object ("stop teaching me about this" is
#: still a stop request). ``i've shown you`` / ``i showed you`` are
#: exact-match only — a conversational "I've shown you the project
#: before" must never close a live capture or answer "wasn't watching".
_TEACH_STOP_STRONG = ("stop teaching", "done teaching", "end the lesson")
_TEACH_STOP_GENERIC = ("that's it", "thats it", "that's all",
                       "thats all", "i'm done", "im done", "i am done",
                       "we're done", "we are done", "learned it",
                       "i've shown you", "i have shown you", "i showed you")


def _teach_start_target(text: str) -> str:
    """The workflow name from a teach-start utterance, or "".

    Conservative: memory verbs ("teach me to remember that X") and
    "about" phrasing ("teach me about X" is ASK) return "" so those
    keep their normal routing.
    """
    m = _TEACH_START_RE.search(text or "")
    if not m:
        return ""
    target = m.group(1).strip()
    lower = target.lower()
    # Memory verbs ("teach me to remember that X") and "about" phrasing
    # ("teach me about X" is ASK) are NOT demonstrations — keep their
    # normal routing.
    if lower.startswith(("remember ", "remember that", "forget ",
                         "note ", "recall ", "about ")):
        return ""
    # Strip leading fillers so "teach me how to do the deploy routine"
    # names the workflow "deploy routine", not "the deploy routine" or
    # "do the deploy routine".
    target = re.sub(r"^(?:do\s+)?(?:the|a|an)\s+", "", target,
                    flags=re.I).strip()
    if not target or target.lower() in ("this", "that", "it", "them",
                                        "those"):
        return ""
    return target[:60]


def _teach_stop_kind(text: str) -> str:
    """'strong' | 'generic' | '' — the completion-phrase kind."""
    lower = re.sub(r"[.!?]+$", "", (text or "").strip()).lower()
    for p in _TEACH_STOP_STRONG:
        if lower.startswith(p):
            return "strong"
    if lower in _TEACH_STOP_GENERIC:
        return "generic"
    return ""
...
#: ``screen_handler(text) -> str`` — routes SCREEN intents ("what's on
#: my screen", "click the login button", "type hello", "scroll
#: down") to the Wave 23 screen layer (OCR + input simulation). Falls
#: through to desktop control for everything else, so the two layers
#: compose without breaking existing desktop phrases.
ScreenHandler = Callable[[str], str]
...
class TextCommandHandler:
    ...
    def __init__(self, conn=None,
                 ...
                 screen_handler: Optional[ScreenHandler] = None,
                 cwd: Optional[str] = None,
                 llm: Optional[object] = None,
                 vault_root: Optional[str] = None,
                 md_skills: Optional[object] = None) -> None:
        self.conn = conn
        self.desktop_handler = desktop_handler
        #: Wave 23 — Friday's eyes and hands. None → the screen layer
        #: degrades to desktop control (screen tools missing is honest).
        self.screen_handler = screen_handler
        self.cwd = cwd
        #: The ONE NLU point's LLM client (Wave 13a). None → deterministic
        #: fallback inside resolve().
        self.llm = llm
        #: Vault root for the MemoryFact bridge (Wave 1). None → the
        #: product default (~/.friday/v6_vault); hermetic tests pass a
        #: tmp root.
        self.vault_root = vault_root
        #: Markdown-skill library (Wave 2). None → the product default
        #: (bundled + ~/.friday/v6_skills); hermetic tests inject a tmp
        #: library so operator-skill overrides route through the brain.
        self.md_skills = md_skills
    ...
    def _route(self, raw: str, *, confirm_fn: ConfirmFn, force: bool,
        ...
        try:
            ...
            # Screen commands (Wave 23) — "what's on my screen", "click
            # the login button", "type hello", "scroll down". Pre-
            # dispatch because the resolver has no screen verbs yet and
            # would misroute them ("click the login button" reads as
            # DESKTOP → the desktop layer web-searches "login button").
            # Conservative: only explicit screen phrases fire; everything
            # else falls through to desktop/other routing untouched.
            try:
                from .screen.parsers import parse_screen_intent
                if parse_screen_intent(raw) is not None:
            ...
            # Vault in natural language (Wave 0 → NL): "find X in my
            # vault" / "search the vault for X" / "list my vault notes"
            # / "rebuild the index" — pre-dispatch because the resolver
            # misroutes them ("find" → DESKTOP web search, "search the
            # vault" → PLAN mission). The vault is operator memory and
            # must answer from language on every surface.
            try:
                if _vault_search_terms(raw) or any(
                        p in raw.lower() for p in _VAULT_LIST_PHRASES
                        + _INDEX_REBUILD_PHRASES):
                    return self._vault_nl_response(raw)
            except Exception:
                pass

            # Markdown skills (Wave 2) — discovery + explicit invocation
            # are pre-dispatch: they must win regardless of how the LLM
            # classifies the phrase ("list your skills" can read as
            # unknown; "use the schedule skill to add standup" as
            # PLAN). Discovery lists the bundled/operator SKILL.md
            # files; invocation routes the skill body through the
            # documented Claude path with a deterministic floor. Skill
            # self-creation ("create a skill called X that does Y")
            # authors a SKILL.md into the operator skills dir.
            try:
                from .skills.markdown import MarkdownSkillLibrary
                lib = self.md_skills or MarkdownSkillLibrary()
                if _is_skill_overview_phrase(raw):
                    return self._skill_overview_response(raw)
                created = self._skill_create_response(raw)
                if created is not None:
                    return created
                skill = lib.explicit_name(raw)
                if skill is not None:
                    return self._markdown_skill_response(raw, skill)
            except Exception:
                pass

            # "teach me to do X" — the watch-me loop closes in language.
            # Pre-dispatch because the resolver misroutes these ("teach
            # me to do deploy" → PLAN mission, "i'll show you how" →
            # ASK, "that's it" → unknown). Conservative: memory verbs
            # fall through ("teach me to remember that X" stays MEMORY),
            # and generic completions ("that's it") only fire when a
            # capture is actually open — a bare "that's it" in ordinary
            # chat is never hijacked.
            try:
                if _teach_start_target(raw) or _teach_stop_kind(raw):
                    ...
                    if conn is None:
                        ...
                        own = True
                    try:
                        result = self._teach_response(raw, conn)
    ...
    def _screen(self, text: str) -> TalkResult:
        """SCREEN intents (Wave 23) — see and touch the desktop.

        The screen layer is another surface of the same brain: it
        answers "what's on my screen" with real OCR and routes clicks /
        typing / scrolling through the input simulator. Falls through to
        desktop control when the screen layer can't handle the phrase
        (missing tools → honest "I can't see", never a crash).
        """
        if self.screen_handler is None:
            return self._desktop(text)
        try:
            response = self.screen_handler(text) or ""
            return TalkResult(text, "screen", "screen",
                              response=response or "Done.")
        except Exception as exc:
            logger.warning(f"screen handler failed: {exc}")
            return TalkResult(text, "screen", "failed",
                              response=f"Screen action failed: {exc}")
    ...
    def _ask_response(self, text: str, action) -> TalkResult:
        ...
        follow-ups get the same conversation context as ``friday6 ask``.
        """
        ...
        # "what do you know about X" — the vault is the operator's
        # readable memory (Wave 1). Answer from evidence (fact rows +
        # vault notes) before the reasoning engine; never fabricate.
        vault_answer = self._vault_knowledge_response(text)
        ...
        try:
            ...
            reasoning_available = True  # answered honestly "I don't know"
        except Exception as exc:
            logger.debug(f"reasoning unavailable ({exc}) — status fallback")
            reasoning_available = False
        # Degraded path: proactive status line (never a lie) — ONLY when
        # the reasoning engine itself is unavailable. When reasoning is
        # available and honestly answers "I don't know", that honest
        # reply — which points at the teach loop (Wave 2c) — must win:
        # a proactive status line is context, not an answer, and would
        # hijack the teach-loop pointer whenever the environment is
        # noisy (dirty repo, recent commits, morning session).
        ...
        return TalkResult(text, "ask", "chat",
                          response="I don't know yet — I don't have "
                                   ...
                                   "project status, recent activity, or "
                                   "mission progress — or teach me: say "
                                   "'teach me to do X' and I'll watch "
                                   "you and learn it.")


    def _vault_knowledge_response(self, text: str) -> Optional[TalkResult]:
        """Answer 'what do you know about X' from the vault + fact rows.

        Wave 1: evidence-cited, never fabricated — matches facts whose
        subject/predicate/value contains the term, plus vault note
        hits. Honest when nothing matches. Returns None when the
        phrase isn't a vault question (the caller keeps routing).
        """
        lower = (text or "").lower()
        term = None
        for phrase in _VAULT_ASK_PHRASES:
            if phrase in lower:
        ...
        try:
            ...
            fact_lines: list[str] = []
            for fact in bridge.recall(subject="operator", limit=1000):
                hay = f"{fact.subject} {fact.predicate} {fact.value}"
                if term.lower() in hay.lower():
                    src = f" (from {fact.source})" if fact.source else ""
                    fact_lines.append(
                        f"- {fact.predicate}: {fact.value}{src}")
            note_lines = bridge.vault.search(term, limit=5)
        except Exception as exc:
            logger.debug(f"vault knowledge ask failed: {exc}")
            return TalkResult(
                text, "ask", "chat",
                response="I couldn't read my vault right now.")
        if not fact_lines and not note_lines:
            return TalkResult(
                text, "ask", "chat",
                response=(f"I don't have anything on '{term}' yet — say "
                          f"'remember that …' to teach me."))
        parts: list[str] = []
        if fact_lines:
            parts.append(f"Here's what I remember about {term}:")
            parts.extend(fact_lines)
        if note_lines:
            parts.append("And from my vault:" if fact_lines
                         else f"From my vault about {term}:")
            parts.extend(f"  {n}" for n in note_lines)
        return TalkResult(
            text, "ask", "chat", response="\n".join(parts), goal=term)
        ...
        if not suggestions:
            return TalkResult(
                ...
                response="I don't have a pending suggestion right now — "
                         "try 'what should I do next' or 'friday6 skills "
        ...
        if outcome.action == "denied":
            return TalkResult(
                ...
                response=(f"The first step needs an explicit override — I "
                          ...
                          f"is saved; use friday6 talk --force to run it."),
    ...
    def _memory_response(self, text: str, action) -> TalkResult:
        ...
        the statement becomes a proposition through the MemoryFact
        bridge (Wave 1): the SQLite ``memories`` row (subject
        ``operator``, provenance ``talk``) AND a vault wiki note
        carrying the ``sources:`` evidence frontmatter. "forget X"
        removes both. Storing never fabricates — the operator's words
        are the fact.
        """
        if self.conn is None:
            return TalkResult(
        ...
        if not fact_text:
            return TalkResult(
                ...
                response="What should I remember? Try 'remember that I "
                         ...
                         "push'.")
        try:
            from .memory import DECAY_TIME
            bridge = self._memory_bridge()
            predicate = self._memory_predicate(fact_text)
            result = bridge.remember("operator", predicate, fact_text,
        ...
        if result is None:
            return TalkResult(
                text, "memory", "failed",
                response="I couldn't store that.")
        response = "Noted — I'll remember that."
        if result.get("note"):
            from pathlib import Path as _P
            response += f" (vault: {_P(result['note']).name})"
        response += " Say 'forget that' to remove it."
        return TalkResult(
            text, "memory", "memory", response=response, goal=fact_text)
    ...
    def _memory_fact_text(self, text: str, action) -> str:
        ...
        if goal and goal.strip() and goal.strip().lower() not in (
            ...
            return goal.strip()
        lower = text.lower()
        # "teach me to remember that X" is in the MEMORY vocabulary (the
        # resolver scores it MEMORY) — its fact is the statement after the
        # teach frame, not the frame itself.
    ...
    def _memory_bridge(self):
        """The MemoryFact bridge (vault ⇄ DB) for this handler."""
        from .vault import MemoryFact, Vault
        return MemoryFact(conn=self.conn, vault=Vault(root=self.vault_root))

    def _memory_predicate(self, fact_text: str) -> str:
        """A stable, readable fact key from the statement: the first
        up-to-3 significant words ('I prefer Rust for tooling' →
        ``prefer-rust-tooling``). Fallback ``note``. Gives each
        remembered thing its own DB row + wiki note."""
        _STOP = {"the", "and", "for", "that", "this", "with", "you",
                 "your", "are", "was", "from", "have", "about", "like"}
        words = [w for w in re.split(r"\W+", (fact_text or "").lower())
                 if len(w) >= 3 and w not in _STOP]
        return "-".join(words[:3]) or "note"

    def _memory_forget(self, text: str, fact_text: str) -> TalkResult:
        """Forget a stored fact (subject-scoped, honest when empty).
        Removes the SQLite row(s) AND the vault note(s) (Wave 1)."""
        try:
            bridge = self._memory_bridge()
            removed = False
            if fact_text:
                # Forget any operator fact matching the phrase.
                for fact in bridge.recall(subject="operator", limit=1000):
                    if (fact_text.lower() in fact.value.lower()
                            or fact_text.lower() in fact.key.lower()):
                        removed = bridge.forget(
                            fact.subject, fact.predicate) or removed
            else:
                removed = bridge.forget("operator")
        except Exception as exc:
        ...
        if not skills:
            return TalkResult(
                ...
                response="I haven't formed any skills yet. Say 'watch me "
                         "do this' or 'teach me to do X' and I'll learn "
                         "from you.")
        lines = "; ".join(
    ...
    def _teach_response(self, text: str, conn) -> Optional[TalkResult]:
        """'teach me to do X' — the watch-me loop closes in language.

        When Friday doesn't know something, the operator *teaches* by
        doing: "teach me to do deploy" / "i'll show you how to do the
        deploy routine" opens a demonstration capture named after the
        workflow; "that's it" / "done teaching" / "stop teaching"
        closes it and auto-forms a shadow skill from the captured
        actions (never executes until verified + promoted).

        Conservative by design:
        - "teach me to remember that X" stays MEMORY fact storage
          (``_teach_start_target`` returns "" for memory verbs).
        - generic completions ("that's it") only fire when a capture
          is actually open — a bare "that's it" in ordinary chat is
          never hijacked.

        Returns None when the utterance isn't a teach phrase (the
        caller keeps routing).
        """
        target = _teach_start_target(text)
        if target:
            from .skills import WatchRecorder
            watcher = WatchRecorder(conn)
            wid = watcher.start(name=target)
            if not wid:
                return TalkResult(text, "skill", "failed",
                                  response="I couldn't start watching.")
            # Wave 23 — the screen side of "watch me": while the watch
            # is open, Friday also samples the screen (screenshot + OCR)
            # so the demo captures the on-screen flow, not just the
            # audited commands. Best-effort: no screen tools → honest
            # "I can't see your screen" note, never a crash. The
            # recorder's thread is daemon; it is stopped on "that's it".
            recorder = self._start_screen_recorder(wid, conn)
            note = ""
            if recorder is not None and not recorder.available:
                note = " (screen capture unavailable — I'll watch your " \
                       "commands only)"
            return TalkResult(
                text, "skill", "watching",
                response=(f"I'm watching — go ahead, show me how you do "
                          f"'{target}'. I'm also watching your screen{note}. "
                          f"When you're done, say 'that's it'."),
                goal=target)
        kind = _teach_stop_kind(text)
        if not kind:
            return None
        return self._teach_stop_response(text, conn, kind)

    def _teach_stop_response(self, text: str, conn,
                             kind: str) -> Optional[TalkResult]:
        """End a teaching session — form a shadow skill from the capture.

        ``kind`` is the completion-phrase strength from
        ``_teach_stop_kind``. A ``generic`` completion ("that's it")
        with no open capture is ordinary chat and returns None (the
        caller keeps routing); a ``strong`` stop ("stop teaching") is
        answered honestly even when idle.
        """
        from .skills import WatchRecorder
        watcher = WatchRecorder(conn)
        if watcher.active() is None:
            ...
            return TalkResult(
                text, "skill", "chat",
                response="I wasn't watching anything — say 'teach me to "
                         "do X' and I'll watch you.")
        # Wave 23: stop the screen sampler so the final observation is
        # captured before the skill forms (its screen-context steps).
        self._stop_screen_recorders()
        formed = watcher.stop()
        if not formed:
            return TalkResult(
                text, "skill", "chat",
                response="I didn't catch any actions to learn from — "
                         "try 'teach me to do X' then show me the thing.")
        skill = formed["skill"]
        count = formed.get("actions", 0)
        screen_steps = [s for s in skill.steps
                        if s.get("action_type") == "screen"]
        screen_note = (f" with {len(screen_steps)} screen observation(s)"
                       if screen_steps else "")
        return TalkResult(
            text, "skill", "skill_formed",
            response=(f"I watched {count} action(s) and formed the "
                      f"skill '{skill.name}' ({len(skill.steps)} step(s)"
    ...
    #: Active screen demo recorders by watch id (Wave 23). The teach
    #: path starts one sampler per watch; "that's it" stops them all
    #: before the skill forms. Never-crash: a stuck sampler is skipped.
    _screen_recorders: dict = {}

    def _start_screen_recorder(self, watch_id: str, conn):
        """Start the screen sampler for a watch (best-effort, never raises).

        Returns the recorder (or None). The recorder reports
        ``available`` honestly — no screen tools means "I'll watch your
        commands only", never a crash.
        """
        try:
            from .screen.recorder import ScreenDemoRecorder
            recorder = ScreenDemoRecorder(watch_id, conn=conn)
            recorder.start()
            self._screen_recorders[watch_id] = recorder
            return recorder
        except Exception as exc:
            logger.debug(f"screen recorder start failed: {exc}")
            return None

    def _stop_screen_recorders(self) -> None:
        """Stop every active screen sampler (called before skill formation).

        The final observation is captured by ``stop()`` so the formed
        skill's screen-context steps end on what the demo ended with.
        """
        for wid, recorder in list(self._screen_recorders.items()):
            try:
                recorder.stop()
            except Exception as exc:
                logger.debug(f"screen recorder stop failed: {exc}")
        self._screen_recorders.clear()

    def _markdown_skill_response(self, text: str, skill) -> TalkResult:
        """Run an explicit markdown-skill invocation (Wave 2).

        The skill body is the *documented Claude path*: when the
        ``CLAUDE:`` bridge is available, its persistent session
        receives the operator's request prefixed with the matched
        skill's SKILL.md (name + description + body) so Claude follows
        the skill's how-to. Without the bridge, the deterministic
        floor answers from the skill file itself — its description and
        the body's first rules — honest, never fabricated, never a
        dead-end (the degrade path the design doc's W2 exit criterion
        demands).
        """
        # Claude path: embed the skill instructions into the persistent
        # bridge session.
        try:
            ...
            if bridge.available():
                prompt = (
                    f"Use this skill to do the following:\n\n"
                    f"{skill.description}\n\n{skill.body}\n\n"
                    f"Operator request: {text}")
                result = bridge.send(prompt)
                if result.get("ok"):
                    return TalkResult(
                        text, "skill", "skill_routed",
                        response=(f"Routed to Claude Code with the "
                                  f"'{skill.name}' skill — follow the "
                                  f"Live feed for its progress."),
                        goal=skill.name)
                # Honest failure: the bridge is present but could not
                # connect/accept (worker dead, auth, model unreachable).
                # The deterministic floor below still answers — but the
                # operator must hear WHY Claude didn't take it, or a
                # silently-dropped skill looks like a broken Friday.
                response = result.get("response") or ""
                if response:
                    return TalkResult(
                        text, "skill", "skill_failed",
                        response=(f"The Claude bridge couldn't take the "
                                  f"'{skill.name}' skill: {response} "
                                  f"Here's what the skill says: "
                                  f"{skill.description}"),
                        goal=skill.name)
        except Exception as exc:
            logger.debug(f"markdown skill claude path skipped: {exc}")
        # Deterministic floor: the skill's own words.
        rules = [ln.strip().lstrip("-*# ") for ln in skill.body.splitlines()
                 if ln.strip().startswith(("- ", "* "))]
        hint = " ".join(rules[:2])
        response = (f"I have the '{skill.name}' skill: {skill.description}"
                    + (f" It says: {hint}." if hint else "")
                    + f" (source: {skill.source})")
        return TalkResult(text, "skill", "skill", response=response,
                          goal=skill.name)

    def _skill_overview_response(self, text: str) -> TalkResult:
        """'what skills do you have' — markdown skills + learned ones.

        Wave 2 discoverability: the bundled/operator SKILL.md files are
        listed alongside the shadow-first learned workflows, each with
        its source (bundled vs operator-authored). Honest when empty;
        never crashes when either layer is unavailable.
        """
        md_skills: list[str] = []
        try:
            from .skills.markdown import MarkdownSkillLibrary
            lib = self.md_skills or MarkdownSkillLibrary()
            for s in lib.list():
                md_skills.append(f"{s.name} ({s.source})")
        except Exception as exc:
            logger.debug(f"skill overview: md unavailable ({exc})")
        learned: list[str] = []
        try:
            from . import db as _db
            conn = self.conn
            ...
            try:
                from .skills import SkillRegistry
                for s in SkillRegistry(conn).list(limit=20):
                    learned.append(f"{s.name} ({s.verification_state})")
            finally:
                if own:
                    conn.close()
        except Exception:
            pass
        if not md_skills and not learned:
            return TalkResult(
                text, "skill", "chat",
                response="I don't have any skills yet. Say 'watch me do "
                         "this' or 'teach me to do X' to teach me a "
                         "workflow, or drop a SKILL.md into "
                         "~/.friday/v6_skills/.")
        parts: list[str] = []
        if md_skills:
            parts.append("Markdown skills: " + ", ".join(md_skills)
                         + ". Say 'use the <name> skill …' to run one.")
        if learned:
            parts.append("Learned workflows: " + ", ".join(learned) + ".")
        return TalkResult(text, "skill", "chat",
                          response=" ".join(parts))

    def _vault_nl_response(self, text: str) -> TalkResult:
        """Vault operations in natural language (Wave 0 → NL).

        "find X in my vault" searches (index, grep fallback), "list my
        vault notes" lists, "rebuild the index" reindexes. The vault is
        operator memory — the same vault the MemoryFact bridge writes —
        so these answer from the operator's own notes. Honest when
        empty; never crashes when the vault is missing.
        """
        from .vault import Vault
        vault = Vault(root=self.vault_root)
        lower = text.lower()
        try:
            if any(p in lower for p in _INDEX_REBUILD_PHRASES):
                from .vault import VaultIndex
                index = VaultIndex(root=self.vault_root)
                count = index.rebuild()
                if count == 0 and not index.fts_available():
                    response = "Index rebuilt — FTS5 isn't available on " \
                               "this Python build, so search falls back " \
                               "to grep."
                else:
                    response = (f"Index rebuilt — {count} document(s) "
                                f"indexed.")
                return TalkResult(text, "vault", "vault",
                                  response=response)
            if any(p in lower for p in _VAULT_LIST_PHRASES):
                notes = vault.list_wiki()
                if not notes:
                    return TalkResult(
                        text, "vault", "vault",
                        response="The vault is empty — say 'remember that "
                                 "…' or 'vault note <name> <text>' to "
                                 "store something.")
                names = ", ".join(p.stem for p in notes)
                return TalkResult(
                    text, "vault", "vault",
                    response=(f"{len(notes)} note(s) in the vault: "
                              f"{names}."))
            terms = _vault_search_terms(text)
            if terms:
                lines, source = vault.search_with_source(terms, limit=10)
                if not lines:
                    return TalkResult(
                        text, "vault", "vault",
                        response=(f"Nothing in the vault matches '{terms}' "
                                  f"— say 'remember that …' to teach me."))
                source_label = "index" if source == "index" else "grep"
                joined = "; ".join(lines[:5])
                response = (f"{len(lines)} match(es) for '{terms}' "
                            f"(via {source_label}): {joined}.")
                return TalkResult(text, "vault", "vault",
                                  response=response, goal=terms)
        except Exception as exc:
            logger.warning(f"vault nl failed: {exc}")
            return TalkResult(
                text, "vault", "failed",
                response="I couldn't read the vault right now.")
        return TalkResult(text, "vault", "chat",
                          response="I couldn't make sense of that vault "
                                   "request — try 'find rust in my vault' "
                                   "or 'list my vault notes'.")

    def _skill_create_response(self, text: str) -> Optional[TalkResult]:
        """"create a skill called X that does Y" — Friday extends itself.

        Authors a SKILL.md into the operator skills dir
        (``~/.friday/v6_skills/<name>/SKILL.md``) so the new skill is
        immediately discoverable and invocable ("use the <name> skill")
        on every surface. Never crashes: an unwritable dir degrades to
        an honest error, never a false success. Returns None when the
        utterance isn't a create-skill request.
        """
        m = _SKILL_CREATE_RE.search(text or "")
        if m is None:
            return None
        name = m.group(1).strip()
        # Description = what follows the *matched* name ("create a skill
        # called X that runs the tests" → "that runs the tests"). Slice
        # from the regex match's end — never ``lower.find``, which would
        # take the first occurrence of the name in the sentence.
        rest = text[m.end():].strip()
        rest = re.sub(r"^(that|which|to|for)\s+", "", rest,
                      flags=re.IGNORECASE).strip()
        description = rest or f"Operator-defined '{name}' skill."
        try:
            from .skills.markdown import DEFAULT_USER_SKILLS_DIR
            user_dir = getattr(self.md_skills, "user_dir", None) \
                or DEFAULT_USER_SKILLS_DIR
            skill_dir = user_dir / name
            skill_dir.mkdir(parents=True, exist_ok=True)
            body = _skill_body_from_description(name, description)
            (skill_dir / "SKILL.md").write_text(body, encoding="utf-8")
        except Exception as exc:
            logger.warning(f"skill create failed: {exc}")
            return TalkResult(
                text, "skill", "failed",
                response=f"I couldn't create that skill: {exc}")
        return TalkResult(
            text, "skill", "skill_created",
            response=(f"Created the '{name}' skill — say 'use the "
                      f"{name} skill' to run it."),
            goal=name)
