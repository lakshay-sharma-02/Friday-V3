"""CLI commands for `friday6 mobile` — companion app + transport (Wave 15/7).

Usage:
    friday6 mobile serve [--host 127.0.0.1] [--port 8900] [--db PATH]
    friday6 mobile push  [--once] [--db PATH]    # drain the durable queue
...
logger = logging.getLogger("friday_v6.cli_mobile")
...
def cmd_mobile_pair(args: argparse.Namespace) -> int:
    ...
    print(f"  {_DIM}Then see what's paired: `friday6 mobile devices`{_RESET}\n")
    return 0
...
def cmd_mobile_devices(args: argparse.Namespace) -> int:
    ...
    if not devices:
        print(f"  {_DIM}No phones paired yet — `friday6 mobile pair` to")
    ...
    print(f"\n  {_DIM}Unpair: `friday6 mobile unpair <device-id>`{_RESET}\n")
    return 0
...
def cmd_mobile_unpair(args: argparse.Namespace) -> int:
    ...
    print(f"  {_DIM}No device with id {args.device_id} (see "
          f"`friday6 mobile devices`).{_RESET}\n")
    return 1
...
def _normalize_bind(host: str, port: int) -> tuple:
    ...
    or a full URL — so pasting what ``friday6 mobile remote`` prints
...
def cmd_mobile_serve(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        print(f"  {_DIM}  Try: friday6 mobile serve --host 0.0.0.0{_RESET}")
        return 1
    port = server.server_address[1]
    # flush=True: the URLs must be visible immediately even when stdout
    # is redirected (`friday6 mobile serve | tee log`), not just on a TTY
    ...
    if host in ("127.0.0.1", "localhost"):
        ...
        print(f"  {_DIM}with `friday6 mobile serve --host 0.0.0.0`{_RESET}", flush=True)
    else:
    ...
    print(f"  {_DIM}Then: `friday6 mobile pair` → enter the code in the app's", flush=True)
    print(f"  {_DIM}Device tab → 'Add to Home Screen' for an app icon.{_RESET}", flush=True)
    print(f"  {_DIM}Reachable from anywhere? `friday6 mobile remote`{_RESET}", flush=True)
...
def cmd_mobile_remote(args: argparse.Namespace) -> int:
    ...
    if not lan and not ts:
        ...
        print(f"  {_DIM}  Start the server: `friday6 mobile serve --host 0.0.0.0`{_RESET}")
    ...
    if ts:
        ...
        print(f"     phone (free), then re-run `friday6 mobile remote` for")
    ...
    if token:
        ...
        print(f"  server with a token first:  friday6 mobile serve "
              f"--host 0.0.0.0 --token \"$(openssl rand -hex 16)\"")
        print(f"  and enter the same token in the phone app's Status tab.")
    print(f"\n  {_DIM}Then pair: `friday6 mobile pair` → code into the app's")
...
def _friday6_command() -> str:
    """How to invoke Friday from an autostart entry.

    Prefers the installed ``friday6`` console script (what the operator
    runs); falls back to ``sys.executable -m friday_v6.cli_talk`` so the
    ...
    *quoted* for the Desktop-Entry Exec field — project/venv paths
    ...
    import shutil
    import sys
    exe = shutil.which("friday6")
    if exe:
        return f'"{exe}"'
    return f'"{sys.executable}" -m friday_v6.cli_talk'


def cmd_mobile_autostart(args: argparse.Namespace) -> int:
    """Write ~/.config/autostart/friday6-mobile.desktop (like 9router).

    The entry launches ``friday6 mobile serve --host 0.0.0.0 --tray``
    ...
    path = os.path.join(d, "friday6-mobile.desktop")
    content = (
        "[Desktop Entry]\n"
        "Type=Application\n"
...
def cmd_mobile_push(args: argparse.Namespace) -> int:
    ...
    # when it's running, a manual `friday6 mobile push` mostly drains
...
def build_mobile_parser(subparsers) -> None:
    """Register `friday6 mobile` (used by the integrated CLI)."""
    ...
    autostart = mobile_sub.add_parser(
        ...
        description="Writes an XDG autostart entry (~/.config/autostart/"
                    "friday6-mobile.desktop) that launches "
                    "`friday6 mobile serve --host 0.0.0.0 --tray` when you "
    ...
    no_autostart = mobile_sub.add_parser(
        ...
        description="Deletes ~/.config/autostart/friday6-mobile.desktop if "
                    "present (idempotent).")
...
if __name__ == "__main__":  # pragma: no cover - standalone entry
    ...
    parser = argparse.ArgumentParser(prog="friday6 mobile")
