"""CLI commands for `friday6 mission` — persistent goals (Wave 9 + 18).

The missions layer's debug hatch (the Wiring Law: every layer gets its
``friday6 <layer> …`` command; the product surface stays natural
language through ``friday6 talk``).
...
:func:`friday_v6.missions.make_planner` construction point is used, so
this CLI never diverges from what talk/voice/web do.

Usage:
    friday6 mission create "ship the auth refactor by Friday"
    friday6 mission list
    friday6 mission status <id>
    friday6 mission replan <id> [--goal G] [--reason R]
    friday6 mission advance <id> [--force] [--manual-result R]
    friday6 mission start|pause|cancel|complete|delete <id>
"""
    ...
    try:
        ...
        if not missions:
            print(f"  {_DIM}No missions"
                  ...
                  f"yet. Create one: friday6 mission create "
...
def cmd_mission_replan(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        reason = args.reason or "replanned via friday6 mission replan"
...
def build_mission_parser(subparsers) -> None:
    """Register `friday6 mission <cmd>` subcommands."""
    parser = subparsers.add_parser(
        ...
        description="Track goals as persistent missions with ordered, "
                    ...
                    "when opted in) exactly like friday6 talk.",
    )
...
def main(argv: Optional[list[str]] = None) -> int:
    """Standalone entry (used by tests + `python -m friday_v6.cli_missions`)."""
    parser = argparse.ArgumentParser(prog="friday6 mission")
