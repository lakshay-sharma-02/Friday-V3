...
logger = logging.getLogger("friday_v6.skills.watcher")
...
def _generalized_steps(actions: list[dict]) -> list[dict]:
    ...
    matching generalizes across sessions: ``pytest -q`` in ``friday_v6``
...
def _screen_event_steps(events: list[dict]) -> list[dict]:
    """Shape screen observations like audited actions for capture.

    Wave 23: while a watch was open, Friday also *saw* the screen
    (periodic OCR snapshots in ``screen_events``). Those observations
    become ``action_type="screen"`` steps whose command is the OCR
    text — informational context in the formed skill (what screen the
    demo happened on), never a command the dispatcher will run.
    ``created_at`` is carried through for chronological interleaving.
    """
    steps: list[dict] = []
    for e in events or []:
        ...
        steps.append({
            "action_type": "screen",
            "command": text[:200],
            "goal": (e.get("event_type") or "screen_snapshot").strip(),
            "cwd": "",
            "created_at": e.get("created_at", ""),
        })
    return steps
...
def _ordered_capture(actions: list[dict], desktop: list[dict],
                     screen: list[dict] | None = None) -> list[dict]:
    """Interleave audited actions + desktop + screen events in real order.

    All lists arrive oldest-first. Merging by source would put every
    audited action before every app open / screen state even when they
    happened mid-demonstration. Each item is tagged with its
    ``created_at`` and stable sort order (rowid within a source), then
    everything is zipped by timestamp. Desktop events are pre-shaped via
    ``_desktop_event_steps`` and screen observations via
    ``_screen_event_steps`` so they carry the same keys
    ``_generalized_steps`` expects.
    """
    tagged: list[tuple[str, int, dict]] = []
    for i, a in enumerate(actions or []):
        tagged.append((a.get("created_at", ""), i, a))
    offset = len(actions or [])
    for src, shaper in ((desktop, _desktop_event_steps),
                        (screen, _screen_event_steps)):
        shaped = shaper(src)
        for i, e in enumerate(shaped):
            tagged.append((e.get("created_at", ""), offset + i, e))
        offset += len(shaped)
...
class WatchRecorder:
    ...
    def stop(self, watch_id: Optional[str] = None,
        ...
        desktop = db.desktop_events_between(
            self._conn, watch.get("started_at", ""), db.now_iso()) or []
        # Wave 23 — the screen side of the demonstration: what Friday
        # *saw* while the watch was open (periodic OCR snapshots). Same
        # chronological merge — a screen state that changed mid-demo sits
        # where it happened, not appended at the end.
        screen = db.screen_events_between(
            self._conn, watch.get("started_at", ""), db.now_iso()) or []
        ...
        # capture). ``_ordered_capture`` interleaves all sources on
        # created_at (microsecond-precision, so same-second ties are
        # resolved by real insert order).
        capture = _ordered_capture(actions, desktop, screen)
