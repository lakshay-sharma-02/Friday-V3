"""CLI commands for `friday6 daemon` — the ambient FRIDAY process.

Usage:
    friday6 daemon start [--voice] [--no-notifications] [--poll N]
    friday6 daemon status
    friday6 daemon stop
"""
...
logger = logging.getLogger("friday_v6.cli_daemon")
...
def cmd_daemon_status(args: argparse.Namespace):
    ...
    if not running and status:
        ...
        print(f"\n  {_YELLOW}◐{_RESET} Daemon not running. Start with `friday6 daemon start`.")
...
def build_daemon_parser(subparsers) -> None:
    """Register `friday6 daemon <cmd>` subcommands."""
