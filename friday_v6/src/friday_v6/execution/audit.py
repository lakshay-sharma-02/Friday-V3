...
:class:`AuditLogger` wraps the V4 ``actions`` table (``friday_v6.db``)
so every gate decision and execution attempt leaves a durable trace the
operator can inspect with ``friday6 db status`` / ``friday6 actions``.
...
logger = logging.getLogger("friday_v6.execution.audit")
