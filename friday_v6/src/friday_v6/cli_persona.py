"""CLI commands for `friday6 persona` — identity as a verbatim view (Wave 10).
...
Usage:
    friday6 persona profile              # what Friday knows about you
    friday6 persona remember "call me Lakshay"   # record verbatim
    friday6 persona profile --json
"""
...
logger = logging.getLogger("friday_v6.cli_persona")
...
def cmd_persona_profile(args: argparse.Namespace) -> int:
    """`friday6 persona profile` — what Friday knows about the operator."""
...
def cmd_persona_remember(args: argparse.Namespace) -> int:
    """`friday6 persona remember <text>` — record a statement verbatim.
...
def _add_persona_commands(subparsers) -> None:
    ...
    (`friday6 persona <cmd>`) and the standalone entry point."""
...
def main(argv: list[str] | None = None) -> int:
    """Standalone entry point for `python -m friday_v6.cli_persona`."""
    logging.basicConfig(level=logging.WARNING)
    parser = argparse.ArgumentParser(prog="friday6 persona")
