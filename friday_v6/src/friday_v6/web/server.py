"""Local dashboard server for `friday6 web` — pure-stdlib ``http.server``.
...
logger = logging.getLogger("friday_v6.web.server")
...
<footer>friday6 web · local-first · data from your machine only</footer>
...
async function refresh() {
  try {
    ...
    $("grid").innerHTML = `<div class="card"><h2><span class="led bad"></span>Connection</h2>
      <div class="empty">could not reach the dashboard API — is `friday6 web` still running?</div></div>`;
  }
}
...
class DashboardHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if path == "/api/talk":
            ...
            # execute / missions / reasoning) as voice and `friday6 talk`.
...
def serve(host: str = "127.0.0.1", port: int = 8899) -> None:
    ...
    # redirected (scripting, `friday6 web | tee`), not just on a TTY.
