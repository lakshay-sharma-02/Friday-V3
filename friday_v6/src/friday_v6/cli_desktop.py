"""CLI commands for `friday6 desktop` — desktop awareness & control.

Usage:
    friday6 desktop status              # Show full desktop status
    friday6 desktop windows             # List open windows
    friday6 desktop switch <ws>         # Switch to workspace
    friday6 desktop focus <app>         # Focus an app (natural name or class)
    friday6 desktop launch <app>        # Launch an application
    friday6 desktop screenshot          # Take a screenshot
    friday6 desktop notify <msg>        # Send a desktop notification
    friday6 desktop platforms           # List supported platforms + current DE
    friday6 desktop tray                # Start the system tray icon
    friday6 desktop hotkeys             # Register global hotkeys
    friday6 desktop watch               # Ambient feed → desktop notifications
    friday6 desktop daemon              # Tray + hotkeys + notifier, together
"""
...
logger = logging.getLogger("friday_v6.cli_desktop")
...
def _require_desktop(wm) -> bool:
    ...
    if not wm.is_available:
        ...
        print(f"  {_DIM}  Try: friday6 desktop platforms{_RESET}")
        return False
    return True
...
def _launch_voice_session() -> None:
    ...
    hotkey. Spawns ``friday6 talk`` in the first available terminal
    emulator; falls back to a desktop notification if none is found.
    """
    import shutil
    import subprocess

    cmd = [sys.executable, "-m", "friday_v6.cli_talk", "talk", "--push-to-talk"]
    terminals = [
        ("kitty", ["kitty", "--", *cmd]),
    ...
    print(f"  {_DIM}  🎙 Voice: run `friday6 talk` in a terminal{_RESET}")
    try:
        from .desktop.wm_abstraction import WindowManager
        WindowManager.notify(
            "Friday", "Voice: run `friday6 talk` in a terminal", "normal"
        )
    except Exception:
        pass
...
def cmd_desktop_focus(args: argparse.Namespace) -> int:
    ...
    if resolved:
        ...
        print(f"  {_DIM}  Try: friday6 desktop windows{_RESET}")
        return 1
...
def cmd_desktop_aliases(args: argparse.Namespace) -> int:
    ...
    if not aliases:
        print(f"  {_DIM}No learned apps yet — say 'my <name> is <command>'"
              f" or run: friday6 desktop teach <name> <command>{_RESET}")
        return 0
...
def build_desktop_parser(subparsers) -> None:
    """Build subparser for `friday6 desktop` (used by the integrated CLI).
...
def _build_desktop_subcommands(desktop_sub) -> None:
    """Register `friday6 desktop <cmd>` subcommands on a subparsers object.

    Shared by both the integrated `friday` CLI (via build_desktop_parser)
    and the standalone `python -m friday_v6.cli_desktop` entry point.
    """
    ...
    # friday6 desktop teach <name> <binary>
    p = desktop_sub.add_parser(
...
def main(argv: list[str] | None = None) -> int:
    """Main entry point for `python -m friday_v6.cli_desktop`.

    Uses the same subcommand builder as the integrated CLI but exposes
    the commands directly (no nested ``desktop`` level), so
    ``python -m friday_v6.cli_desktop platforms`` works.
    """
    logging.basicConfig(level=logging.WARNING)

    parser = argparse.ArgumentParser(prog="friday6 desktop")
