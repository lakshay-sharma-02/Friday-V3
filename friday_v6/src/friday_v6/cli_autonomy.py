"""CLI commands for `friday6 autonomy` — Friday's own judgment → action loop.
...
Usage:
    friday6 autonomy status            # loop report (executed / asked / skipped)
    friday6 autonomy pending           # open permission requests
    friday6 autonomy approve <id>      # allow a pending action (like 'yes, run it')
    friday6 autonomy deny <id> [--why "…"]   # decline + record an override
    friday6 autonomy overrides         # what Friday won't propose anymore
    friday6 autonomy clear-overrides [action_type]   # un-block actions
"""
...
logger = logging.getLogger("friday_v6.cli_autonomy")
...
def cmd_autonomy_status(args: argparse.Namespace) -> int:
    """`friday6 autonomy status` — the loop's latest report."""
    ...
    print()
    if payload["pending"]:
        print(f"  {_DIM}  → 'friday6 autonomy pending' to see them, "
...
def cmd_autonomy_pending(args: argparse.Namespace) -> int:
    """`friday6 autonomy pending` — the durable permission asks."""
    conn = _resolve_db(args)
    try:
        from . import db
        if conn is None:
    ...
    for req in pending:
        ...
        print(f"    {_DIM}→ 'friday6 autonomy approve {rid}' or "
...
def cmd_autonomy_approve(args: argparse.Namespace) -> int:
    """`friday6 autonomy approve <id>` — allow a pending action."""
    rid = (args.request_id or "").strip()
    if not rid:
        print(f"  {_RED}✗ give me a request id — 'friday6 autonomy "
...
def cmd_autonomy_deny(args: argparse.Namespace) -> int:
    """`friday6 autonomy deny <id>` — decline + record an override."""
    rid = (args.request_id or "").strip()
    if not rid:
        print(f"  {_RED}✗ give me a request id — 'friday6 autonomy "
              f"pending' lists them.{_RESET}")
...
def cmd_autonomy_overrides(args: argparse.Namespace) -> int:
    ...
    print(f"  {_DIM}  → clear with 'friday6 autonomy clear-overrides "
...
def cmd_autonomy_clear_overrides(args: argparse.Namespace) -> int:
    """`friday6 autonomy clear-overrides [action_type]` — un-block."""
    conn = _resolve_db(args)
    try:
        from . import db
        if conn is None:
...
def _add_autonomy_commands(subparsers) -> None:
    ...
    p.add_argument("request_id", help="Request id from 'friday6 autonomy pending'")
...
def main(argv: list[str] | None = None) -> int:
    """Standalone entry point for `python -m friday_v6.cli_autonomy`."""
    logging.basicConfig(level=logging.WARNING)
    parser = argparse.ArgumentParser(prog="friday6 autonomy")
