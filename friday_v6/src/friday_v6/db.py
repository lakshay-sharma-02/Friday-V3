...
logger = logging.getLogger("friday_v6.db")
...
_MIGRATIONS: list[tuple[int, object]] = [
    ...
    # Wave 23 — screen observations during a watch ("watch me do this").
    # ``screen_events`` rows are what Friday *saw* while a watch was
    # open: periodic OCR snapshots of the screen state. The watch
    # bridge turns them into screen-context skill steps (informational
    # — never auto-executed) so a demo captures the on-screen flow,
    # not just the audited commands.
    (
        10,
        """
        CREATE TABLE IF NOT EXISTS screen_events (
            ...
            watch_id    TEXT NOT NULL DEFAULT '',
                -- the watch this observation belongs to ('' = ambient)
            event_type  TEXT NOT NULL DEFAULT 'screen_snapshot',
                -- screen_snapshot | screen_change
            text        TEXT NOT NULL DEFAULT '',
                -- the OCR text Friday saw (bounded, e.g. 600 chars)
            context     TEXT NOT NULL DEFAULT '',
                -- extra context (focus note / skill hint)
            created_at  TEXT NOT NULL
        );
        CREATE INDEX IF NOT EXISTS idx_screen_events_watch
            ON screen_events(watch_id);
        CREATE INDEX IF NOT EXISTS idx_screen_events_created
            ON screen_events(created_at);
        """,
    ),
]
...
def db_path_of(conn) -> Optional[str]:
    """The main database file path behind a connection, or None.

    Lets a background worker open its OWN connection to the same
    database (threads must never share a caller's connection — and
    must never fall back to the product default behind a hermetic
    test's back). ``None`` for in-memory DBs.
    """
    try:
...
def record_screen_event(conn: sqlite3.Connection,
                        text: str = "", event_type: str = "screen_snapshot",
                        watch_id: str = "", context: str = "",
                        event_id: Optional[str] = None) -> Optional[str]:
    """Record an observed screen state (Wave 23 watch bridge).

    The capture side of "watch me": while a watch is open, Friday
    periodically samples the screen (screenshot + OCR) and stores what
    it saw here, tagged with the watch id. Never raises — returns the
    event id or None. ``text`` is bounded (600 chars) so a busy screen
    never bloats the row.
    """
    eid = event_id or _new_id()
    ts = now_iso()
    ok = _execute(
        conn,
        "INSERT INTO screen_events (id, watch_id, event_type, text, "
        "context, created_at) VALUES (?, ?, ?, ?, ?, ?)",
        (eid, watch_id, event_type, (text or "")[:600], context, ts),
    )
    return eid if ok else None


def screen_events_between(conn: sqlite3.Connection,
                          start_iso: str, end_iso: str) -> list[dict]:
    """Screen observations inside a time window (oldest first).

    The capture side of the watch bridge: what Friday saw while a
    watch was open becomes demonstration material (screen-context
    steps in the formed skill).
    """
    return _fetchall(
        conn,
        "SELECT * FROM screen_events WHERE created_at >= ? AND created_at <= ? "
        "ORDER BY created_at ASC, rowid ASC",
        (start_iso, end_iso),
    )


def recent_screen_events(conn: sqlite3.Connection,
                         limit: int = 20) -> list[dict]:
    """Most recent screen observations (newest first) — context."""
    return _fetchall(
        conn,
...
def recent_exchange_history(conn: sqlite3.Connection,
    ...
    Wave 13: ``friday6 ask`` and the talk/voice router thread this into
...
def db_status(path: Optional[Path] = None) -> dict:
    ...
    Powers ``friday6 db status`` / the unified ``friday6 status`` db row.
    """
