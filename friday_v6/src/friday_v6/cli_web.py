"""CLI commands for `friday6 web` — the local dashboard server.

Usage:
    friday6 web [--host 127.0.0.1] [--port 8899]
...
logger = logging.getLogger("friday_v6.cli_web")
...
def cmd_web(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        print(f"  {_DIM}  Try: friday6 web --port 8900{_RESET}")
        return 1
...
def build_web_parser(subparsers) -> None:
    """Register `friday6 web` (used by the integrated CLI)."""
...
if __name__ == "__main__":  # pragma: no cover - standalone entry
    ...
    parser = argparse.ArgumentParser(prog="friday6 web")
    parser.add_argument("--host", default="127.0.0.1",
                        help="Bind address (default: 127.0.0.1)")
    parser.add_argument("--port", type=int, default=8899,
                        help="Port to listen on (default: 8899)")
