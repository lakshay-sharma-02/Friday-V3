...
bounded timeout — the gated execution layer (``friday6 ide run``) is
...
logger = logging.getLogger("friday_v6.desktop.ide.controller")
...
def run_command(command: str, cwd: Optional[str | Path] = None) -> tuple[bool, str]:
    ...
    Prefer the gated execution layer (``friday6 ide run`` goes through
