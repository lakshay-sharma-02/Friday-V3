"""`friday6 ide` — the IDE integration surface (Wave 6).

One command to see what Friday knows about your editor and your code:

    friday6 ide                      status summary
    friday6 ide detect               which editor is present (and how to reach it)
    friday6 ide diagnose FILE        issues in a file (LSP → AST fallback)
    friday6 ide symbols FILE         functions/classes outline
    friday6 ide open FILE            open a file in the editor
    friday6 ide reveal FILE LINE     jump to a line
    friday6 ide run CMD              run a command through the gated execution pipeline
...
logger = logging.getLogger("friday_v6.cli_ide")
...
def cmd_ide(args: argparse.Namespace) -> int:
    """`friday6 ide` — status summary (default subcommand)."""
...
def cmd_detect(args: argparse.Namespace) -> int:
    """`friday6 ide detect` — the editor(s) Friday sees."""
...
def cmd_diagnose(args: argparse.Namespace) -> int:
    """`friday6 ide diagnose FILE` — issues in a file."""
...
def cmd_symbols(args: argparse.Namespace) -> int:
    """`friday6 ide symbols FILE` — the file's outline."""
    _print_logo("Symbols")
    mod = _load()
    p = _resolve_file(args.file, args.cwd)
    try:
...
def cmd_open(args: argparse.Namespace) -> int:
    """`friday6 ide open FILE` — open in the detected editor."""
    _print_logo("Open")
    mod = _load()
    ide = mod.detect()
    if ide is None:
...
def cmd_run(args: argparse.Namespace) -> int:
    """`friday6 ide run CMD` — through the SAME gated execution pipeline.

    The IDE surface composes with execution: the command runs through
    gate → sandbox → audit exactly like `friday6 talk \"run …\"` — and
...
def build_ide_parser(subparsers) -> None:
    """Register `friday6 ide` (Wave 6 IDE integration)."""
