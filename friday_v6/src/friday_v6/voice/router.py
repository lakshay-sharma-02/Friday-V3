...
logger = logging.getLogger("friday_v6.voice.router")
...
class VoiceRouter:
    ...
    def _try_nlu_route(self, lower: str) -> Optional[str]:
        ...
        try:
            ...
            try:
                ...
                llm = None
            # screen_handler is the callable the router invokes
            # (ScreenTextHandler.handle); desktop_handler is the plain
            # interpreter the screen layer falls through to. Passing the
            # handler *instance* as desktop_handler crashed on screen
            # phrases ('ScreenTextHandler' object is not callable).
            screen = self._screen_or_desktop_handler()
            handler = TextCommandHandler(
                conn=self._conn,
                llm=llm,
                screen_handler=screen.handle,
                desktop_handler=lambda t: (
                    self._try_desktop_command(t.lower(), t) or ""))
    ...
    def _try_desktop_command(self, lower: str, original: str) -> Optional[str]:
        # Screen phrases FIRST (Wave 23): "click the login button" would
        # otherwise fall through to the desktop interpreter's web-search
        # fallback ("login button" isn't an app) — the screen handler
        # owns clicks/types/scrolls and falls through to desktop for
        # everything else. Real input actions ask the operator aloud.
        try:
            from ..screen.parsers import parse_screen_intent
            if parse_screen_intent(original) is not None:
                handler = self._screen_or_desktop_handler()
    ...
    def _screen_or_desktop_handler(self):
        """The ONE screen+desktop surface for voice (Wave 23).

        Screen phrases ("what's on my screen", "click the login
        button") are answered with real OCR + input; everything else
        falls through to the same ``desktop_text_command`` interpreter
        the CLI/web/phone use. Real input actions (clicks/types) ask
        the operator aloud first (voice_confirm).
        """
        try:
            from ..nl_router import voice_confirm
            from ..screen import ScreenTextHandler
            return ScreenTextHandler(
                confirm_fn=voice_confirm(self._ask_voice_confirm),
                desktop_fallback=lambda t: (
                    self._try_desktop_command(t.lower(), t) or ""))
        except Exception as exc:
            logger.debug(f"screen handler unavailable ({exc}) — desktop only")
            return lambda t: (self._try_desktop_command(t.lower(), t) or "")
