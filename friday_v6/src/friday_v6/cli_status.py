"""Status probes + `friday6 db status` for the V4 CLI.

The unified ``friday6 status`` command lives in ``cli_doctor.cmd_status``
...
This module contributes the probes plus the ``friday6 db status`` command
...
logger = logging.getLogger("friday_v6.cli_status")
...
#: `friday6 status` command in cli_doctor delegates here — one source of
...
def _probe_daemon() -> tuple[Optional[bool], str]:
    ...
    return False, "not running (start with `friday6 daemon start`)"
...
def _probe_voice() -> tuple[Optional[bool], str]:
    ...
    try:
        import friday_v6.voice  # noqa: F401 — import check only
    except Exception as exc:
...
def _probe_security() -> tuple[Optional[bool], str]:
    ...
    return True, "ready (no scans yet — `friday6 security scan`)"
...
def _probe_web() -> tuple[Optional[bool], str]:
    ...
    try:
        ...
        return None, "not running (`friday6 web`)"
    except Exception as exc:
...
def _probe_mobile() -> tuple[Optional[bool], str]:
    ...
    try:
        ...
        return None, "not running (`friday6 mobile serve`)"
    except Exception as exc:
        return None, f"probe failed: {exc}"
...
def cmd_status(args: argparse.Namespace) -> int:
    """Unified V4 layer overview (the `friday6 status` command).
...
def build_db_parser(subparsers) -> None:
    """Register `friday6 db` (Wave 9.0 state database management)."""
...
if __name__ == "__main__":  # pragma: no cover - standalone entry
    ...
    parser = argparse.ArgumentParser(prog="friday6 db")
