...
logger = logging.getLogger("friday_v6.daemon")
...
class IntelligenceSampler:
    ...
    sampler feeds real metrics while the daemon runs so `friday6
...
class SkillLearner:
    ...
    requires the operator's explicit approval (``friday6 skills promote``).
...
class MobilePushWorker:
    ...
    without manual ``friday6 mobile push``.
    ...
    def poll_once(self) -> int:
        ...
        try:
            ...
            # to each registered Expo token (no manual `friday6 mobile
...
class SecurityScanner:
    ...
    ``friday6 doctor`` / ``friday6 status`` can report the last scan
