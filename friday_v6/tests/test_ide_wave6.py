...
from friday_v6.reasoning.question import QuestionType, classify as qclassify
...
def force_ast(monkeypatch):
    ...
    monkeypatch.setattr(
        "friday_v6.desktop.ide.lsp_command_for_workspace", lambda *a, **k: None)
    return monkeypatch
...
class TestLSPClient:
    def test_initialize_and_pull_diagnostics(self, fake_lsp):
        from friday_v6.desktop.ide.lsp_client import LSPClient
    ...
    def test_diagnostic_brief_is_one_line(self):
        from friday_v6.desktop.ide.lsp_client import Diagnostic
    ...
    def test_server_closing_mid_session_raises_lsp_error(self, tmp_path):
        ...
        at ``_request``."""
        from friday_v6.desktop.ide.lsp_client import LSPClient, LSPError, uri_for
...
class TestASTAnalyzer:
    def test_syntax_error(self):
        from friday_v6.desktop.ide.ast_analyzer import analyze_source
    ...
    def test_analyze_file_missing_returns_empty(self, tmp_path):
        from friday_v6.desktop.ide.ast_analyzer import analyze_file
...
class TestAnalyzeFile:
    def test_ast_fallback_when_no_server(self, tmp_path, force_ast):
        from friday_v6.desktop.ide import analyze_file
    ...
    def test_lsp_path_when_server_resolvable(self, tmp_path, fake_lsp,
        ...
        target.write_text("x = 1\n\n\nSENTINEL_ISSUE\n")
        monkeypatch.setattr(
            "friday_v6.desktop.ide.lsp_command_for_workspace",
    ...
    def test_symbols_ast_outline(self, tmp_path, force_ast):
        from friday_v6.desktop.ide import symbols
        f = tmp_path / "mod.py"
    ...
    def test_lsp_command_lookup(self, tmp_path, monkeypatch):
        from friday_v6.desktop.ide import lsp_command_for_workspace
        (tmp_path / "pyproject.toml").write_text("")
        monkeypatch.setattr(
            "friday_v6.desktop.ide._find",
    ...
    def test_no_marker_no_server(self, tmp_path, monkeypatch):
        from friday_v6.desktop.ide import lsp_command_for_workspace
        monkeypatch.setattr("friday_v6.desktop.ide._find", lambda exe: None)
...
class TestDetection:
    def test_env_vscode(self, monkeypatch):
        from friday_v6.desktop.ide import detection
    ...
    def test_preflight_opt_in(self, monkeypatch):
        from friday_v6.desktop.ide import preflight_opted_in
...
class TestWiring:
    ...
    def test_status_probe_ide(self, monkeypatch):
        from friday_v6 import cli_status
...
class TestClaudePreflight:
    def test_ide_context_appended_when_opted_in(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("FRIDAY_V4_IDE_PREFLIGHT", "1")
        monkeypatch.setattr("friday_v6.execution.executors.find_tool",
...
class TestCLI:
    ...
    def test_cli_ide_importable(self):
        from friday_v6.cli_ide import (build_ide_parser, cmd_ide,
    ...
    def test_package_importable(self):
        import friday_v6.desktop.ide as ide
