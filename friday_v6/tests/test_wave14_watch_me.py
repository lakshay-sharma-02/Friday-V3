...
def _record_demo(conn, repo: str = "friday_v6", rounds: int = 2) -> None:
...
class _NoScreen:
    """A screen that is never available — never a real display.

    The Wave 23 teach path ("teach me to do X") starts a
    ``ScreenDemoRecorder``. On machines with grim/import/tesseract
    installed, the REAL recorder samples the live screen during the
    test — a stray OCR read can fabricate a screen observation and turn
    an honest "didn't catch any actions" into a formed skill
    (environment-flaky, and ~30s per real OCR). Injecting an
    unavailable screen keeps every test here hermetic: recording always
    degrades to nothing (same pattern as ``test_screen_recorder.py``).
    """
    ...
    def ocr(self, image_path=None):
        ...
        return ActionResult(False, "screen unavailable")


@pytest.fixture(autouse=True)
def _hermetic_screen(monkeypatch):
    """Every test in this file must stay hermetic — no real display.

    ``ScreenDemoRecorder.__init__`` is patched to inject an unavailable
    fake screen so the recorder the teach path *actually* creates can
    never sample the operator's screen (``start()`` returns False, no
    thread, nothing recorded). Mirrors the injectable-screen pattern in
    ...
    def patched_init(self, watch_id, conn=None, screen=None,
        ...
        real_init(self, watch_id, conn=conn, screen=_NoScreen(),
                  interval=interval, db_path=db_path)

    monkeypatch.setattr(ScreenDemoRecorder, "__init__", patched_init)
...
class TestWatchesDb:
    def test_schema_v7_includes_watches_desktop_and_autonomy(self, tmp_path):
        conn = _conn(tmp_path)
        try:
            # v10 (Wave 23) adds screen_events on top of v9 — the
            # watches/desktop/autonomy tables from v7..v9 are still there.
...
class TestWatchRecorder:
    def test_watch_stop_forms_generalized_shadow_skill(self, tmp_path):
        ...
        try:
            ...
            _record_demo(conn, repo="friday_v6", rounds=1)  # one workflow
            ...
            assert skill.steps[0]["repo"] == "friday_v6"
...
class TestWatchCapturesDesktop:
    ...
    def test_watch_captures_app_opens(self, tmp_path):
        ...
        try:
            ...
            db.record_desktop_event(conn, app="code", repo="friday_v6")
            formed = watcher.stop(wid)
            assert formed
            skill = formed["skill"]
            steps = skill.steps
    ...
    def test_engine_observer_records_desktop_events(self, tmp_path):
        ...
        try:
            ...
            assert events[0]["repo"] == "friday_v6"
...
class TestRepetitionNoticer:
    def test_notices_repeated_pattern(self, tmp_path):
        conn = _conn(tmp_path)
        try:
            _record_demo(conn, repo="friday_v6", rounds=3)
            ...
            assert offer["context"] == "friday_v6"  # repo generalization
        finally:
            conn.close()
...
class TestSkillRouter:
    ...
    def test_watch_then_learn_forms_skill(self, tmp_path):
        ...
        conn = handler.conn
        _record_demo(conn, repo="friday_v6")
    ...
    def test_skill_name_hint_parses_utterance(self):
        from friday_v6.nl_router import _skill_name_hint
...
# ==========================================================================
# "teach me to do X" — the watch-me loop closes in natural language
# ==========================================================================


class TestTeachPath:
    """"teach me to do X" starts a capture; "that's it" forms the skill.

    Pre-dispatch: the resolver misroutes these ("teach me to do X" →
    PLAN mission, "i'll show you how" → ASK, "that's it" → unknown), so
    the teach hook wins before intent dispatch. Safety: memory verbs
    fall through ("teach me to remember X" stays MEMORY), and generic
    completions ("that's it") only fire when a capture is open.
    """
    ...
    def test_teach_me_starts_capture(self, tmp_path):
        handler = self._handler(tmp_path)
        result = handler.handle("teach me to do deploy")
        assert result.action == "watching"
        assert "deploy" in result.response
        watch = db.active_watch(handler.conn)
        assert watch is not None
        assert watch["name"] == "deploy"

    def test_teach_me_how_to_variant(self, tmp_path):
        handler = self._handler(tmp_path)
        result = handler.handle("teach me how to do the deploy routine")
        assert result.action == "watching"
        assert db.active_watch(handler.conn)["name"] == "deploy routine"

    def test_show_you_starts_capture(self, tmp_path):
        handler = self._handler(tmp_path)
        result = handler.handle("i'll show you how to do the deploy routine")
        assert result.action == "watching"
        assert "deploy routine" in result.response

    def test_teach_then_that_s_it_forms_shadow_skill(self, tmp_path):
        handler = self._handler(tmp_path)
        start = handler.handle("teach me to do deploy")
        assert start.action == "watching"
        _record_demo(handler.conn, repo="friday_v6", rounds=1)
        done = handler.handle("that's it")
        assert done.action == "skill_formed"
        assert "shadow" in done.response
        skills = SkillRegistry(handler.conn).list()
        assert skills and skills[0].name == "deploy"
        assert skills[0].verification_state == STATE_SHADOW
        assert db.active_watch(handler.conn) is None  # capture closed

    def test_done_teaching_forms_skill(self, tmp_path):
        handler = self._handler(tmp_path)
        handler.handle("teach me to do deploy")
        _record_demo(handler.conn)
        done = handler.handle("done teaching")
        assert done.action == "skill_formed"
        assert "shadow" in done.response

    def test_generic_completion_without_watch_not_hijacked(self, tmp_path):
        """A bare "that's it" with no open capture is ordinary chat —
        it must NOT stop anything or form a skill (and never crash)."""
        handler = self._handler(tmp_path)
        result = handler.handle("that's it")
        assert result.action != "skill_formed"
        assert result.action != "watching"
        assert db.active_watch(handler.conn) is None

    def test_strong_stop_without_watch_is_honest(self, tmp_path):
        handler = self._handler(tmp_path)
        result = handler.handle("stop teaching")
        assert "wasn't watching" in result.response

    def test_conversational_ive_shown_you_not_hijacked(self, tmp_path):
        """"I've shown you the project before" is ordinary chat — it must
        NOT close a live capture or answer "wasn't watching" (exact-match
        only for the shown-you completion)."""
        handler = self._handler(tmp_path)
        result = handler.handle("i've shown you the project before")
        assert "wasn't watching" not in result.response
        # And with a capture open, the trailing clause must not end it.
        handler.handle("teach me to do deploy")
        still = handler.handle("i've shown you the project before")
        assert still.action != "skill_formed"
        assert db.active_watch(handler.conn) is not None  # capture intact

    def test_empty_capture_completion_is_honest(self, tmp_path):
        """"that's it" with an open capture but zero captured actions →
        honest empty reply pointing at the teach surface, never a crash."""
        handler = self._handler(tmp_path)
        handler.handle("teach me to do deploy")
        result = handler.handle("that's it")
        assert result.action == "chat"
        assert "didn't catch any actions" in result.response
        assert "teach me to do X" in result.response
        assert db.active_watch(handler.conn) is None  # capture closed

    def test_teach_me_to_remember_stays_memory(self, tmp_path):
        """"teach me to remember that X" is MEMORY fact storage, never
        a demonstration capture — the memory-guard must win pre-dispatch."""
        handler = self._handler(tmp_path)
        result = handler.handle("teach me to remember that I prefer Rust")
        ...
        assert len(facts) == 1  # stored as a fact, not a demo

    def test_teach_me_about_is_not_capture(self, tmp_path):
        """"teach me about X" is ASK (explain), not a demonstration."""
        handler = self._handler(tmp_path)
        result = handler.handle("teach me about the vault")
        assert result.action != "watching"
        assert db.active_watch(handler.conn) is None

    def test_i_dont_know_points_at_teach(self, tmp_path):
        """The honest "I don't know" fallback now points at the teach
        loop — closing the loop Friday-side too."""
        handler = self._handler(tmp_path)
        result = handler.handle("what is the meaning of life")
        assert "teach me to do X" in result.response

    def test_target_extraction_unit(self):
        from friday_v6.nl_router import (_teach_start_target,
                                          _teach_stop_kind)
        assert _teach_start_target("teach me to do deploy") == "deploy"
        assert _teach_start_target(
            "teach me how to do the deploy routine") == "deploy routine"
        assert _teach_start_target(
            "i'll show you how to do the deploy routine") == "deploy routine"
        assert _teach_start_target(
            "let me show you the deploy routine") == "deploy routine"
        assert _teach_start_target(
            "teach me to remember that I prefer Rust") == ""  # MEMORY
        assert _teach_start_target("teach me about the vault") == ""  # ASK
        assert _teach_start_target("that's it") == ""
        assert _teach_stop_kind("that's it") == "generic"
        assert _teach_stop_kind("done teaching") == "strong"
        assert _teach_stop_kind("stop teaching now") == "strong"
        assert _teach_stop_kind("i've shown you") == "generic"
        assert _teach_stop_kind("i've shown you the project before") == ""
        assert _teach_stop_kind("i'm done with the vault") == ""
...
class TestSkillsCli:
    ...
    def test_watch_and_watch_stop(self, tmp_path, capsys):
        from friday_v6.cli_skills import cmd_skills_watch, cmd_skills_watch_stop
    ...
    def test_watch_stop_without_watch(self, tmp_path, capsys):
        from friday_v6.cli_skills import cmd_skills_watch_stop
    ...
    def test_noticed_lists_offers(self, tmp_path, capsys):
        from friday_v6.cli_skills import cmd_skills_noticed
...
class TestSkillLearnerOffers:
    def test_sweep_once_reports_offers(self, tmp_path):
        from friday_v6.daemon import SkillLearner
