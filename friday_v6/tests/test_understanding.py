...
The Wave 9 deterministic NLU was replaced by :mod:`friday_v6.nlu` — the
ONE NLU point (LLM-first, deterministic fallback, single ``resolve()``).
``friday_v6.understanding`` is now a *thin shim* that re-exports every
...
from friday_v6 import nlu as real_nlu
from friday_v6 import understanding

from friday_v6.understanding import (
    Assessment,
    Entity,
    EntityType,
    Intent,
