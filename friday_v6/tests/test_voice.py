...
class TestUtils:
    def test_write_and_read_wav_roundtrip(self):
        from friday_v6.voice.utils import read_wav, write_wav
...
class TestConfig:
    def test_load_config_defaults(self):
        from friday_v6.config import load_config
    ...
    def test_write_default_config_matches_dataclass(self, tmp_path):
        ...
        from friday_v6.config import write_default_config
...
class TestTTSUtils:
    def test_auto_voice_mode_alert(self):
        from friday_v6.voice.tts import VoiceMode, auto_voice_mode
    ...
    def test_split_sentences_short_text_single_chunk(self):
        from friday_v6.voice.tts import TextToSpeech
    ...
    def test_chime_generation(self):
        from friday_v6.voice.chimes import get_chime
    ...
    def test_tts_speak_no_provider_returns_false(self):
        from friday_v6.voice.tts import TextToSpeech, TTSConfig
        with patch("friday_v6.voice.tts.PyTTSProvider") as mock_py, \
             patch("friday_v6.voice.tts.EdgeTTSProvider") as mock_edge, \
             patch("friday_v6.voice.tts.KokoroONNXProvider") as mock_kokoro:
...
class TestChimes:
    def test_chime_playback_doesnt_crash(self):
        from friday_v6.voice.chimes import play_chime
...
class TestPipelineState:
    def test_state_transition_calls_callback(self):
        from friday_v6.voice.pipeline import PipelineConfig, PipelineState, VoicePipeline
    ...
    def test_start_stop_lifecycle(self):
        ...
        from friday_v6.voice.pipeline import PipelineConfig, VoicePipeline
...
class TestSTT:
    def test_stt_result_dataclass(self):
        from friday_v6.voice.stt import STTResult
...
class TestHotword:
    def test_hotword_init_graceful(self):
        from friday_v6.voice.hotword import HotwordDetector
    ...
    def test_energy_detector_basic(self):
        from friday_v6.voice.hotword import EnergyDetector
    ...
    def test_openwakeword_empty_keyword_graceful(self):
        from friday_v6.voice.hotword import OpenWakeWordProvider
...
class TestVAD:
    def test_vad_init_graceful(self):
        from friday_v6.voice.vad import VoiceActivityDetector
    ...
    def test_silero_frame_audio_pads_480_to_512(self):
        ...
        from friday_v6.voice.vad import SileroVAD
        vad = SileroVAD()
...
class TestAudio:
    ...
    def test_audio_stream_init(self):
        from friday_v6.voice.audio import AudioStream
        stream = AudioStream()
...
class TestRouter:
    def test_router_init(self):
        from friday_v6.voice.pipeline import PipelineConfig, VoicePipeline
        from friday_v6.voice.router import VoiceRouter
        pipeline = VoicePipeline(PipelineConfig())
    ...
    @patch("friday_v6.desktop.wm_abstraction.WindowManager")
    @patch("friday_v6.desktop.wm_abstraction.shutil.which")
    ...
    def test_router_ask_reaches_reasoning(self):
        ...
        from friday_v6.nl_router import TalkResult
        ...
        with patch("friday_v6.nl_router.TextCommandHandler", _FakeHandler):
            pipeline = VoicePipeline(PipelineConfig())
            router = VoiceRouter(pipeline, enable_proactive=False)
    ...
    def test_router_persists_spoken_utterances_with_conn(self, tmp_path):
        ...
        from friday_v6.persona import recent_statements
        from friday_v6.voice.pipeline import PipelineConfig, VoicePipeline
        from friday_v6.voice.router import VoiceRouter
...
class TestPushToTalk:
    def test_bind_push_to_talk_with_keyboard(self):
        ...
        from friday_v6.cli_talk import _bind_push_to_talk
