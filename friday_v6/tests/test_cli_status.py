"""Hermetic tests for `friday6 status` and `friday6 db status`.
...
from friday_v6.cli_status import (
    cmd_db_status,
    cmd_status,
    _probe_daemon,
    _probe_db,
...
class TestCmdStatus:
    ...
    def test_status_returns_nonzero_when_down(self, capsys, monkeypatch):
        _stub_probes(monkeypatch)
        monkeypatch.setattr(cli_status, "_probe_web",
                            lambda: (False, "not running (friday6 web)"))
        rc = cmd_status(argparse.Namespace())
        out = capsys.readouterr().out
        assert rc == 1
...
class TestParsers:
    def test_doctor_registers_status_and_doctor(self):
        """`friday6 status` is registered once, by cli_doctor, pointing at
