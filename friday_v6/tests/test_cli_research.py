...
from friday_v6 import cli_research
...
def test_cli_research_main_standalone(repo: Path, capsys):
    """`friday6 research analyze X` works (regression: the standalone
...
def test_nl_research_routes_to_correlate(tmp_path: Path, capsys):
    """Law 1: `friday6 talk "analyze X vs Y"` reaches the research layer."""
    from friday_v6 import db
    conn = db.connect(tmp_path / "v4.db")
    a = tmp_path / "vivaha"
    b = tmp_path / "mindwell"
